...
static void require_kv_arena_plan(const ds4_stage_info *s, const char *label) {
    char msg[128];
    uint64_t off = 0;
...
static void fill_good_facts(ds4_device_fact facts[DS4_V100_EXPECTED_GPUS]) {
    memset(facts, 0, sizeof(ds4_device_fact) * DS4_V100_EXPECTED_GPUS);
...
static void test_classification(void) {
    char err[256];
    ds4_policy p;
    require_true(ds4_classify_or_die("bf16", "source_bf16",
    ...
    require_true(ds4_classify_or_die("f8_e4m3_b128",
    ...
    require_true(ds4_classify_or_die("mxfp4", "source_mxfp4_grouped",
    ...
    require_true(ds4_classify_or_die("f32", "source_f32",
                                          "v100_fp32_gemm",
                                          &p, err, sizeof(err)) != 0,
                 "f32 model gemm fallback must fail policy");
    require_true(ds4_classify_or_die("f32", "source_f32_matmul",
                                          "ds4_control",
                                          &p, err, sizeof(err)) != 0,
...
static void test_layer_map(void) {
    require_true(ds4_stage_for_layer(-1) == -1, "negative layer invalid");
    require_true(ds4_stage_for_layer(0) == 0, "layer 0 gpu0");
    require_true(ds4_stage_for_layer(5) == 0, "layer 5 gpu0");
    require_true(ds4_stage_for_layer(6) == 1, "layer 6 gpu1");
    require_true(ds4_stage_for_layer(34) == 5, "layer 34 gpu5");
    require_true(ds4_stage_for_layer(42) == 7, "layer 42 gpu7");
    require_true(ds4_stage_for_layer(43) == -1, "layer 43 invalid");
    require_true(ds4_layer_class_for_layer(0) == DS4_V100_LAYER_SWA_ONLY,
                 "layer 0 swa-only class");
    require_true(ds4_layer_class_for_layer(1) == DS4_V100_LAYER_SWA_ONLY,
                 "layer 1 swa-only class");
    require_true(ds4_layer_class_for_layer(2) == DS4_V100_LAYER_RATIO_4,
                 "layer 2 ratio-4 class");
    require_true(ds4_layer_class_for_layer(3) == DS4_V100_LAYER_RATIO_128,
                 "layer 3 ratio-128 class");
    require_true(!strcmp(ds4_layer_class_name(DS4_V100_LAYER_RATIO_4), "ratio_4"),
                 "ratio-4 class name");
}
...
static void test_topology_fail_closed(void) {
    ...
    require_true(ds4_context_open(&ctx, &opts, err, sizeof(err)) != 0,
...
static void test_pack_binding(void) {
    ...
    require_true(ds4_context_tensor_count(ctx) == 5, "descriptor count");
    require_true(ds4_context_has_token_embedding(ctx), "token embedding descriptor");
    require_true(ds4_context_exec_count(ctx, DS4_V100_EXEC_DIAGNOSTIC_ONLY) == 1,
                 "diagnostic exec count");
    require_true(ds4_context_exec_count(ctx, DS4_V100_EXEC_F32_CONTROL) == 2,
                 "f32 control exec count");
    require_true(ds4_context_exec_count(ctx, DS4_V100_EXEC_F16_HMMA) == 1,
                 "f16 hmma exec count");
    require_true(ds4_context_exec_count(ctx, DS4_V100_EXEC_LOWBIT_KERNEL) == 1,
                 "lowbit exec count");
    const ds4_stage_info *s0 = ds4_context_stage(ctx, 0);
    ...
    const ds4_layer_info *l0 = ds4_context_layer(ctx, 0);
...
static void test_turbomind_pack_binding(void) {
    ...
    require_true(ds4_context_tensor_count(ctx) == 5,
    ...
    ds4_turbomind_binding tm;
    require_true(ds4_context_require_layer_turbomind_binding(
...
static void test_kv_budget_math(void) {
    ...
    ds4_kv_budget b = ds4_kv_budget_for_layer(0, ctx, 1);
    ...
    b = ds4_kv_budget_for_layer(2, ctx, 1);
    ...
    b = ds4_kv_budget_for_layer(3, ctx, 2);
...
static void test_kv_stage_admission(void) {
    ...
    const ds4_layer_info *l0 = ds4_context_layer(ctx, 0);
    const ds4_layer_info *l2 = ds4_context_layer(ctx, 2);
    const ds4_layer_info *l3 = ds4_context_layer(ctx, 3);
    ...
    const ds4_stage_info *s1 = ds4_context_stage(ctx, 1);
