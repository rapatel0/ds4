...
int main(void) {
    ...
    if (ds4_cuda_context_relay_smoke(ctx, 0, 0, DS4_V100_RELAY_F16,
                                          1, err, sizeof(err))) {
        fprintf(stderr, "cuda_hc_relay_smoke: f16 loopback: %s\n", err);
        ds4_cuda_context_close(ctx);
        return 1;
    }
    if (ds4_cuda_context_relay_smoke(ctx, 0, 0, DS4_V100_RELAY_F32_DEBUG,
                                          1, err, sizeof(err))) {
    ...
    if (opts.expected_gpus >= 2) {
        ...
        if (ds4_cuda_context_relay_smoke(ctx, 0, 1, DS4_V100_RELAY_F16,
                                              1, err, sizeof(err))) {
