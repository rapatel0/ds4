...
#   ART_DIR      artifact root (default /workspace/s603-artifacts)
...
# (notably DS4_V100_TP_EP_S602_SYNC=join|edges, the per-point bisect
# overrides DS4_V100_TP_EP_S602_SYNC_E0/_E1/_E2, and the
# DS4_V100_TP_EP_S602_* calibration/verify envs).
set -u
name="$1"; shift || true
requests="${REQUESTS:-128}"
tokens="${TOKENS:-64}"
slots="${SLOTS:-32}"
art="${ART_DIR:-/workspace/s603-artifacts}"
log="$art/$name"
ctl="${CONTROL_DIR:-/workspace/s597-phase01-artifacts/phase0-full-control}"
# wait-for-idle preflight (s598..s601 discipline)
ok=0
...
[ "$ok" = "1" ] || { echo "s603-run: GPUs not idle, aborting" >&2; exit 1; }
fp=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader | sed '/^$/d' | wc -l)
[ "$fp" = "0" ] || { echo "s603-run: foreign GPU processes present, aborting" >&2; nvidia-smi; exit 1; }
cd /workspace/ds4
...
if [ -n "$case_dir" ]; then
  ...
  echo "=== s600/s601/s602 flag lines in server.log:"
  grep -h "tp_ep_s602\|tp_ep_s601\|tp_ep_s600\|tp_ep_nccl" "$case_dir/server.log" 2>/dev/null | head -30 || true
fi
exit $rc
