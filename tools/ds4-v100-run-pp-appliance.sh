...
usage() {
    cat <<'USAGE'
usage: tools/ds4-v100-run-pp-appliance.sh [options]
...
The PP/base launcher reads DS4_V100_* variables from the environment file,
validates the deployment contract, then execs tools/ds4-v100-replay --serve.
USAGE
}

fail() {
    echo "ds4-v100-run-pp-appliance: $*" >&2
    exit 1
}

warn() {
    echo "ds4-v100-run-pp-appliance: warning: $*" >&2
}
...
if [ -n "${DS4_V100_SERVE_MODE_LOCK:-}" ] &&
   [ "${DS4_V100_SERVE_MODE_LOCK:-}" != "base" ]; then
    fail "DS4_V100_SERVE_MODE_LOCK must be base for the PP launcher"
fi
