...
__global__ void resident_op_kernel(float * dst, size_t elems, int repeats, float norm) {
    const size_t i = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= elems) {
        return;
    }
...
void usage(const char * argv0) {
    std::fprintf(stderr,
                 ...
                 "       [--root-index 0..3] [--algo root|doubling|nccl]\n"
...
void run_root_collective(const Options & opt, float ** inputs, float ** outputs, float * staging,
...
void run_nccl_allreduce(const Options & opt, float ** inputs, float ** outputs,
                        cudaStream_t streams[kParticipants], ncclComm_t comms[kParticipants],
                        size_t elems) {
    NCCL_CHECK(ncclGroupStart());
    for (int p = 0; p < kParticipants; ++p) {
        CUDA_CHECK(cudaSetDevice(opt.devices[p]));
        NCCL_CHECK(ncclAllReduce(inputs[p], outputs[p], elems, ncclFloat, ncclSum,
                                 comms[p], streams[p]));
    }
...
void run_resident_ops(const Options & opt, float ** outputs, cudaStream_t streams[kParticipants],
                      size_t elems) {
    const int block = 256;
    const int grid = (int) ((elems + block - 1) / block);
...
void run_layer_proxy(const Options & opt, float ** buf0, float ** buf1, float ** recv,
                     float * staging, cudaStream_t root_stream,
                     cudaStream_t streams[kParticipants], size_t elems, size_t bytes,
                     ncclComm_t comms[kParticipants], float *** final_buf) {
    float ** cur = buf0;
    float ** next = buf1;
...
float verify_outputs(const Options & opt, float ** outputs, size_t elems, size_t bytes) {
    ...
    for (int p = 1; p < kParticipants; ++p) {
        ...
        for (size_t i = 0; i < elems; ++i) {
            if (!std::isfinite(ref[i]) || !std::isfinite(h[i])) {
                return std::numeric_limits<float>::infinity();
            }
            max_abs = std::max(max_abs, std::fabs(h[i] - ref[i]));
        }
    }
...
int main(int argc, char ** argv) {
    ...
    float * buf0[kParticipants] = {};
    float * buf1[kParticipants] = {};
    ...
    for (int i = 0; i < opt.warmup; ++i) {
        run_layer_proxy(opt, buf0, buf1, recv, staging, root_stream, streams, elems, bytes,
                        comms, &final_buf);
    }
    ...
    std::printf("ds4-v100-tp4-layer-proxy algo=%s devices=%d,%d,%d,%d root=%d "
                "tokens=%d hidden=%d bytes_per_tensor=%zu layers=%d "
                "collectives_per_layer=%d local_op_repeats=%d warmup=%d iters=%d\n",
                algo_name(opt.algo), opt.devices[0], opt.devices[1], opt.devices[2],
                opt.devices[3], root_dev, opt.tokens, opt.hidden, bytes, opt.layers,
