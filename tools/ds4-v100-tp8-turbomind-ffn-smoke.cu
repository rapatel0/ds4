    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::strcmp(arg, "--lib") == 0) {
            ...
            if (!val || !parse_devices(val, opt->devices)) {
                std::fprintf(stderr, "invalid --devices; expected eight unique ids\n");
                return false;
            }
            ++i;
...
int compare_outputs(const Options & opt, const DeviceSide & full, const float * d_sum,
                    int total_routes) {
    const size_t elems = (size_t) total_routes * kHidden;
    std::vector<__half> h_full(elems);
    std::vector<float> h_sum(elems);
    CHECK_CUDA(cudaSetDevice(opt.devices[0]));
    CHECK_CUDA(cudaMemcpy(h_full.data(), full.d_down, elems * sizeof(__half),
                          cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_sum.data(), d_sum, elems * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (size_t i = 0; i < elems; ++i) {
        const float a = __half2float(h_full[i]);
        const float b = h_sum[i];
        ...
        err2 += (double) diff * (double) diff;
        denom += (double) a * (double) a;
        if (diff > 2.5e-2f && diff > 8.0e-2f * std::fabs(a)) {
            ++bad;
        }
    }
    const double rel = std::sqrt(err2 / std::max(denom, 1.0e-30));
    const double bad_frac = (double) bad / (double) elems;
    const bool fail = nan || rel > 1.5e-2 || bad_frac > 5.0e-4;
    std::printf("correctness routes=%d values=%zu max_abs=%.6g rel=%.6g bad=%d "
                "bad_frac=%.6g nan=%d %s\n",
...
int main(int argc, char ** argv) {
    ...
    for (int p = 0; p < kParticipants; ++p) {
        ...
        if (p > 0 && api.init(opt.devices[p]) != 0) {
            std::fprintf(stderr, "ggml_turbomind_init failed on device %d\n", opt.devices[p]);
            return 3;
        }
    ...
    for (int i = 0; i < opt.warmup; ++i) {
        for (int p = 0; p < kParticipants; ++p) {
            ...
            char label[64];
            std::snprintf(label, sizeof(label), "tp8 shard warmup p=%d", p);
    ...
    (void) run_reduce_to_root(opt, sides, d_sum, d_recv, total_routes, true);

    const int correctness = compare_outputs(opt, full, d_sum, total_routes);
    ...
    std::printf("ds4-v100-tp8-turbomind-ffn-smoke devices=");
    for (int p = 0; p < kParticipants; ++p) {
    ...
    std::printf("latency_ms full=%.6f tp8_compute=%.6f tp8_reduce=%.6f tp8_total=%.6f "
