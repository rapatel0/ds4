...
#   ART_DIR      artifact root (default /workspace/s602-artifacts)
...
# (notably DS4_V100_TP_EP_HC_TRANSPORT=nccl|kernel and the
...
tokens="${TOKENS:-64}"
slots="${SLOTS:-32}"
art="${ART_DIR:-/workspace/s602-artifacts}"
log="$art/$name"
...
[ "$ok" = "1" ] || { echo "s602-run: GPUs not idle, aborting" >&2; exit 1; }
fp=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader | sed '/^$/d' | wc -l)
[ "$fp" = "0" ] || { echo "s602-run: foreign GPU processes present, aborting" >&2; nvidia-smi; exit 1; }
cd /workspace/ds4
