...
| 55 | A6 PATH 4 failure capture | Active diagnostic | Sprint 481 attempted the rank-major attention projection PATH 4 revive and backed it out after the candidate returned `0/256` HTTP 200. Sprint 482 keeps A6 default-off, adds a distinct diagnostic `--true-ds4-attention-projection-rank-major-input-gate`, wires launcher/profile env, and makes the profile harness emit `failure-summary.{json,md}` on early server failures. Use this to capture the server-side failure before any new promotion A/B. |
