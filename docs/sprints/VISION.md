...
| 2026-05-29 | Sprint 550 reduced compact EP pack padding work. | Changed `ep_pack_route_dest_shards_kernel` from a flat `routes * hidden` launch to one block per graph-visible route. Active routes write the same packed values; inactive padded routes return before hidden-wide work. Remote build passed in `/workspace/s550-ep-pack-route-block`; short selected-token graph sanity passed with `http_200=8`, first token `29361`, `graph_audit_blocker=none`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. | Promote the route-block compact EP pack kernel. Treat the short run as correctness/transport evidence only. Continue C1 by auditing the next fixed-capacity executor/compose padding site or return to typed-KV/full-capture device-state work if remaining padding is inside the grouped-GEMM executor ABI. |
