...
After Sprints 199-201, the near-term practical-serving branch is sharper:
graph-backed `fused6_reduce` is the promoted baseline, easy six-route
kernel-wrapper cut-ins are rejected, and full-layer TP4/EP remains viable only
...
branch should pause.
Sprint 203 built that resident slice and moved the uncertainty to the actual
boundary: correctness passes, but naive root reduction is still slower than
one-GPU routed FFN (`0.825x` at `96 routes x 43 layers`, `0.589x` at
`768 routes x 43 layers`), and the simple sequential doubling variant is worse.
...
Sprint 213 closes the first routed-FFN return path: a materialized six-route
split-reduce diagnostic. It is correct, graph-compatible, and mildly positive
in the focused six-route FFN benchmark (`0.1391 ms` atomic versus `0.1290 ms`
materialized), but not material in served mode: 16-slot/256K continuation moves
only from `60.236036` to `60.655009` tok/s with `16/16` token match and zero
...
- Sprints 112-116 tested hot-path cleanup around the fused appliance.
  ...
  Sprint 152 completed the stage-count software-pipeline sweep and found
  2/3/4-stage gate/up fixed probes neutral at both 768 and 1536 routes.
  Sprint 153 then added the bounded TP appliance-pack contract:
  `--emit-tp-split` can emit split gate/up and down TurboMind descriptors,
  context binding accepts the TP rows, and a layer-3 GPU0/GPU3 bounded pack
...
- Sprint 183 added the first attention/KV-path selector from that evidence:
  ...
  long-context lever.
- Sprint 184 added synthetic token prompt replay, which fixes a measurement
  blind spot: previous 256K runs configured long-context capacity but still used
  an 18-token text prompt. The new `--synthetic-prompt-token` /
  `--synthetic-prompt-len` path can advance real KV position without giant text
  files. A bounded len-256 run recorded `20108.067 ms` prompt replay and
...
- Sprint 186 completed the 4096-token synthetic filled-context tier. Prompt
  replay took `288102.638 ms`, prompt throughput was `14.217155` tok/s, and the
  one continuation step measured `13.354373` tok/s. This confirms prompt replay
  dominates long-context measurement wall time and that full 256K synthetic
  prefill should be scheduled deliberately or preceded by prompt/prefill
  optimization.
...
| 2026-05-21 | Completed Sprint 159 256K slot-scaling check. | A guarded `DS4_V100_EXPERIMENTAL_CTX_SLOT_CAP` override now allows explicit cluster tests above the 256K production cap without changing defaults. 24/32-slot 256K soaks fit in VRAM and preserved selected-token correctness, but throughput was not repeatably better than the 16-slot sustained baseline (`63.41` generated / `62.42` continuation tok/s): 24-slot repeat was `63.76` / `62.76`, and 32-slot was `65.17` / `64.16`. Keep 16 slots as the 256K default; the next material work should be served batch formation for dense routed-FFN shapes or TP/EP/persistent routed-FFN topology, not more slot widening. | Sprint 160+ |
...
| 2026-05-22 | Completed Sprint 176 two-layer TP/EP routed-FFN span. | The TP/EP overlay now supports bounded layer spans through `DS4_V100_TP_EP_LAYER_COUNT`, layer execution selects TP2 for every layer in the span, and the appliance packer can emit `--tp-split-only` overlays that fit 32 GiB V100 VRAM without duplicating full routed weights. V100 stage smoke proved `tp2_layers=2`, selected-token returned `3136`, and full 16-slot/256K scheduler smoke passed. Served 16-slot/256K A/B was correct but slower: control `71.502728` generated / `70.385497` continuation tok/s; quiet TP2 span `66.290863` / `65.255068`, both `16/16` correct. Keep diagnostic-only. Expanding the same per-layer hidden-transfer overlay is not the next lever; either pursue persistent ownership that avoids returning full hidden state each layer, or return to a true in-GPU persistent routed-FFN executor. | Sprint 177+ |
| 2026-05-22 | Completed Sprint 177 FFN-only microbatch wavefront. | The layer executor can now split HC decode around FFN prep, batch only the routed/shared FFN section, and finish HC output through a default-off `DS4_V100_ASYNC_FFN_WAVEFRONT` serving path. V100 correctness passed after fixing compact scratch-slot pointer-table assumptions, and logs prove denser routed shapes (`slots=2/routes=12`, `slots=4/routes=24`). Served 16-slot/256K A/B was correct but slower: control `70.943330` generated / `69.834841` continuation tok/s; chunk 2 `58.063198` / `57.155960`; chunk 4 `35.577336` / `35.021440`, all `16/16` correct. Keep diagnostic-only. Scheduler-side FFN batching is now ruled out as a material lever; next work should be a true persistent/fused routed-FFN executor or broader persistent TP/EP ownership, not more local batching variants. | Sprint 178+ |
...
| 2026-05-22 | Completed Sprint 191 attention-detail profile buckets. | Added default-off `DS4_V100_PROFILE_ATTENTION_DETAIL=1` and propagated attention projection/cache/softmax/inverse-RoPE/output buckets through layer reports, scheduler reports, replay counters, and JSON. V100 direct synthetic profile on the persistent production pack passed: len-256 reached `14.024529` prompt / `14.659513` continuation tok/s with IDs `3955, 361`, and len-1024 reached `14.425868` / `14.429801` with IDs `926, 926`. The len-1024 profile shows attention `37195.555 ms` (`56.25%` total), with output projection `15478.988 ms`, q/kv projection `11803.892 ms`, softmax `5429.091 ms`, cache/update `3739.496 ms`, and inverse RoPE only `383.503 ms`. Sprint 192 should target the attention output/projection boundary, not standalone RoPE or inter-stage handoff. | Sprint 192+ |
| 2026-05-22 | Rejected Sprint 192 single-slot attention output-A HMMA probe. | Added default-off `DS4_V100_SINGLE_SLOT_ATTN_OUTPUT_A_HMMA=1` plus `DS4_CUDA_F8_HMMA_GROUPED_ATTN_O_SINGLE=1`, routing single-token `attention_output_a` through the existing DS4 grouped-batch HMMA kernel. V100 direct synthetic A/B preserved output IDs but regressed hard: len-256 continuation fell from `14.722744` to `8.842737` tok/s and attention output grew from `3876.448 ms` to `15519.781 ms`; len-1024 continuation fell from `14.577239` to `8.747221` and attention output grew from `15473.716 ms` to `61889.994 ms`. Keep diagnostic-only. The next sprint must stop single-kernel substitutions and implement a larger fused/persistent attention projection/output boundary or persistent TP/EP ownership that changes the execution shape materially. | Sprint 193+ |
| 2026-05-22 | Rejected Sprint 193 single-slot F8-F16 cache cuBLAS gate. | Temporarily tested a default-off `DS4_CUDA_F8_F16_CACHE_SINGLE=1` path plus launcher/env support for resident F8->F16 cache single-token probing; the unsafe runtime changes were removed before commit after validation failed. V100 build passed, but the len-256 / 256K filled-context candidate changed output IDs from `3955, 361` to `201, 5`, so the correctness gate failed before len-1024. The measured candidate also did not move the relevant buckets (`14.575734` control vs `14.714705` continuation tok/s, but wrong IDs; attention output `3887.584 ms` vs `3897.414 ms`). A len-8 shape trace showed the intended `cublas_f16_single` path did not select, so the existing global cuBLAS/F16-cache machinery is not a safe shortcut for single-slot decode. Sprint 194 should pivot to persistent TP/EP ownership or an explicitly correctness-tested fused attention kernel, not more wrapper/cache toggles. | Sprint 194+ |
| 2026-05-23 | Completed Sprint 194 TP/EP topology cost contract. | Added `tools/ds4-v100-tp-estimate`, a local/pod-safe estimator for layer-split, routed-only TP2 overlay, TP2/PP1, TP4/PP1, TP8/PP1, and TP4/PP2 topology envelopes. At the 16-slot / 256K tier, the current layer split moves about `7.000 MiB` per token of interstage HC payload, while the rejected routed-only TP2 overlay moves `21.531 MiB` without changing the dense execution shape. Full TP/EP intentionally moves more (`75.250 MiB` for TP2/PP1, `112.875 MiB` for TP4/PP1, `131.688 MiB` for TP8/PP1), so it is only rational if dense attention, shared FFN, routed experts, and output ownership are all native to the topology. The next implementation sprint should not expand the TP2 overlay; build a bounded full-layer TP4/PP1 prototype or a true monolithic routed-FFN kernel. | Sprint 195+ |
| 2026-05-23 | Completed Sprint 195 TP4 hidden collective smoke. | Added `tools/ds4-v100-tp4-collective-smoke`, a four-GPU CUDA peer-copy all-reduce smoke for the `active_microbatch x hidden` TP payload. It builds on V100 with `CUDA_ARCH=sm_70`, verifies exact reduced outputs on both NVLink islands, and measures the naive root gather/reduce/broadcast floor. The 16-token DS4 decode payload costs about `0.11 ms` and larger payloads top out near `27 GB/s` effective wire bandwidth, well below what a production TP collective should use. Full TP4/PP1 remains viable only with an optimized NCCL/ring/tree/fused collective and should stay inside a four-GPU NVLink island before any 8-GPU topology bet. | Sprint 196+ |
| 2026-05-23 | Completed Sprint 196 TP4 recursive-doubling collective. | Extended `tools/ds4-v100-tp4-collective-smoke` with `--algo root|doubling` and measured a repo-owned parallel pairwise all-reduce on the V100 NVLink islands. Doubling verifies correctly and improves larger payloads substantially: at 1024 tokens it cuts latency from `3.676 ms` root to `1.656 ms` and reaches `81.065 GB/s` effective wire bandwidth. It is still slower for the current 16-token decode payload (`0.134 ms` vs `0.111 ms` root), so TP4 remains a batched/prefill or fused-boundary candidate rather than the next direct production decode path. The next sprint should pivot back to a monolithic routed-FFN or persistent layer boundary while preserving the doubling collective for later TP4 prototypes. | Sprint 197+ |
| 2026-05-23 | Completed Sprint 197 routed-FFN liveness profile contract. | Extended `DS4_V100_TURBOMIND_PROFILE=1` with routed materialization counters for route-expanded activation rows, compact activation rows, `mid_half`, `down_routes`, down-reduce epilogue use, and scratch byte totals. V100 profile smoke on the persistent appliance proved the current `fused6_reduce` path uses compact activation staging on every call, elides `down_routes` through the down-reduce epilogue, and still materializes `mid_half` on every routed FFN call. For the production six-route decode shape, `mid_half` is only `24576` bytes per call, while gate/up and down consume about `46-47%` and `24-25%` of profiled routed-FFN time respectively. The next sprint should target a true persistent/tile-level gate-up-plus-down executor or shift to larger batched/prefill TP4 shapes; simply shrinking the remaining buffer is unlikely to be enough. | Sprint 198+ |
| 2026-05-23 | Completed Sprint 198 graph replay for fused routed executor. | Updated the default-off CUDA graph path so it can capture and replay the current `fused6` / `fused6_reduce` routed executor instead of rejecting all routed-executor modes. The graph key now includes executor mode, graph capture skips the compact active-expert host readback, and verbose logs show graph launches. V100 direct replay on the persistent appliance matched output IDs and improved continuation throughput from `16.022442` to `17.980888` tok/s, with `43` captures, `129` graph launches, and `0` graph failures. This is a positive direct-replay signal, not a default promotion: Sprint 169 showed generic graph replay regressed served mode, so Sprint 199 must run the same-binary 16-slot/256K served A/B before deciding whether graph replay is a practical serving win. | Sprint 199+ |
