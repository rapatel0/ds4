...
| 66 | C1 full-capture replay validation | Validation harness repaired; not serving-promoted | Sprint 558 replaced the no-suffix replay-probe guard with a fresh-state validation path: first same-position request serves eager and captures the full graph as audit/cache state; a later matching request replays the cached full graph from fresh request state. Remote build passed. The reduced sequential `8` slot / `256K` / `1` token probe matched eager selected tokens on both requests (`128819`, `128819`) and replayed `43/43` cached full graphs on the second request, with zero peer/SYS transport. | Treat as harness repair only. The second replay checksum still drifted (`5174931161` eager vs `5002850195` replay), so the next C1 work is checksum-drift localization before full-capture position-key relaxation or no-suffix serving promotion. |
| 67 | C1 full-capture checksum drift | Validation correctness repair complete | Sprint 559 localized the Sprint 558 checksum drift to replay-time host metadata: the full-capture graph includes final-HC kernels, but CUDA graph replay does not execute the host-side `d_final_hc_shard` / `d_hc_scratch_shard` pointer swap performed by eager final-HC expansion. The replay path now mirrors that swap only for no-suffix full capture when the final-HC expansion path actually swaps. Build passed. The reduced sequential full-capture replay now matches eager selected tokens and checksums on both requests (`7238127778`, `5174931161`) with `43/43` cached graph replays, zero invalidations, zero peer/SYS transport, and promoted suffix sanity still passes. | Keep no-suffix full capture diagnostic-only and position-keyed. Next C1 work is emitted topology and row-position metadata before any cross-position cache-key relaxation. |
| 68 | C1 emitted-row host metadata mirror | Readiness repair complete; not serving-promoted | Sprint 560 added a no-suffix full-capture cache-hit replay mirror for compressed attention/indexer emitted-row host metadata: row counters, row-position arrays, loaded flags, and loaded positions now advance after successful replay when compressed KV is active. The served appliance still defaults `true_ds4_compressed_kv_gate=false`, so the branch was validated through a one-off compressed-KV entrypoint rather than a permanent flag. Build passed in `/workspace/s560-emitted-metadata`; served-default validation confirmed `compressed_kv_layers=0` and no-regression with `43/43` full-capture cache-hit replays, zero invalidations, zero peer/SYS, and zero NCCL graph SYS edges. The one-off compressed-KV eager and no-suffix full-capture replay probes both exercised `compressed_kv_layers=86` / `compressed_kv_emitted_layers=42` and matched selected tokens/checksums exactly: `58204`, `109597` and `7265791446`, `79399742586`, with `43/43` cached graph replays on the replay leg. Claude bug-find review confirmed the happy-path loaded-row boolean matches eager algebra. | Keep no-suffix full capture diagnostic-only and position-keyed. Next C1 work is emitted/non-emitted topology graph-stability without adding a permanent flag matrix. |
| 69 | C1 emitted topology graph-stability | Readiness repair complete; not serving-promoted | Sprint 561 made graph-mode compressed-KV emitted/non-emitted topology stable by enqueueing the emitted-row topology whenever decode CUDA graph mode is active and masking emitted kernels/copies from `d_decode_position` on non-emitted positions. Eager keeps the compact host-selected path, and host row counters/position/loaded metadata remain tied to the real host `emitted` value. Build passed in `/workspace/s561-emitted-topology`. A one-off compressed-KV entrypoint validated both ratio-4 cases without adding a permanent flag: emitted position `262083` eager/replay matched tokens `58204`, `109597` and checksums `7265791446`, `79399742586`; adjacent non-emitted position `262084` eager/replay matched tokens `109939`, `107875` and checksums `561376577`, `2841198172`. All replay legs had `43/43` cache hits, zero invalidations, zero peer/SYS, and zero NCCL graph SYS edges. Served-default compressed-KV-off sanity also passed with `compressed_kv_layers=0` and `43/43` replay hits. Claude bug-find flagged the graph-mode typed-KV non-emitted extra work as the main risk, and the non-emitted parity probe covered that exact case. | Keep no-suffix full capture diagnostic-only and position-keyed. Next C1 work is cross-position full-capture cache-key relaxation as a diagnostic parity sprint, not serving promotion. |
| 70 | C1 full-capture cross-position cache-key retry | Rejected; position key remains required | Sprint 562 retried no-suffix full-capture cross-position reuse after Sprint 561. The candidate removed the no-suffix position key from persistent invalidation and replay-probe cache-hit detection without adding a flag, then built successfully in `/workspace/s562-cross-position`. A two-request same-session selected-token probe matched eager and showed request two replaying cached position `262083` at live position `262084` with `43` cache hits and zero position invalidations. A stronger six-request same-session probe failed on request three: eager returned token/checksum `117465` / `17092309830`, while replay returned `2039` / `110810249310`, with later requests diverging too. The candidate code was removed. Promoted suffix/default sanity still passed in `/workspace/s562-cross-position-profile-artifacts/none-s562-served-default-suffix-sanity` with `http_200=2`, `43/43` replay hits, zero invalidations, zero peer/SYS, zero NCCL graph SYS, and `compressed_kv_layers=0`. | Reject the cache-key relaxation. Full capture remains diagnostic-only and position-keyed. Next C1 work is residual captured-position state localization before any further cache-key retry. |
| 71 | C1 residual captured-position localization | First divergence localized | Sprint 563 re-applied the failed Sprint 562 relaxation only in a temporary remote copy at `/workspace/s563-localize` and enabled `--decode-stage-checksum-gate`. The temporary diagnostic build passed. The three-request same-session checksum probe reproduced divergence under the relaxed full-capture path. Occurrence 0 matched; occurrence 1 had zero layer-0 checksum diffs but first diverged at layer 1 `hc_current`, including rank-0 `current_shard` `255270109 -> 252329227`, `current_full` `2061267831 -> 2071866390`, `current_full_rank_major` `2041173666 -> 2024541679`, and `final_hc_shard` `1025894790 -> 500914459`. | Full capture remains position-keyed. Next C1 work is layer-1 HC-current replay state repair: localize and fix the inter-layer current/HC handoff after layer-0 full-capture cache-hit replay before retrying cache-key relaxation. |
| 72 | C1 full-capture HC rebase and serving metrology | Positive opt-in signal; not default-promoted yet | Sprint 568 fixed the no-suffix full-capture cross-position bug by recording captured final-HC input/output buffer addresses and rebasing live HC state into the captured input buffer before graph replay. Six-request eager-vs-full-graph selected-token/checksum parity passed with `43` captures, `215` cache-hit replays, and zero invalidations. Sprint 569 then ran serving metrology at `32` slots / `256K` with startup/readiness and a full-slot warmup excluded from the measured window. Deterministic long-prompt serving output matched `32/32` measured generated token sequences between the promoted suffix-control leg and opt-in no-suffix full capture. No-suffix full capture improved request-window generated throughput `12.603435 -> 16.807308` tok/s, continuation throughput `12.209578 -> 16.282079` tok/s, decode generated throughput `1.506832 -> 2.288130` tok/s, and median latency `81.205441s -> 60.873505s`. Response checksum vectors differed, but generated `token_ids` matched exactly and are the parity source for this serving comparison. | Treat no-suffix full capture as correctness-clean with a strong warmed-serving performance signal. Do not flip the launcher default from a single metrology run. Next C1 work is a longer steady-state promotion gate with deterministic generation, startup/init excluded, substantial warmup, and explicit graph/status counter extraction. |
| 73 | C1 no-suffix full-capture longer promotion gate | Rejected; long-generation parity failed | Sprint 570 extended the serving gate to `64` generated tokens/request, `2` full-slot warmup batches, and `4` full-slot measured batches (`128` measured requests). Remote build passed in `/workspace/s570-full-capture-promotion`. Both legs returned `128/128` HTTP 200, replayed `28724` persistent graph lines with `28724` cache-hit/replay-success lines, and had zero peer-copy/SYS hits. Performance remained positive: continuation request-window throughput improved `16.618822 -> 20.814267` tok/s, continuation decode throughput improved `1.513915 -> 2.247000`, and median latency improved `132.083580s -> 103.958728s`. Correctness failed: `ds4_v100.generated_token_sequence` diverged for `128/128` measured responses, and generated text diverged for `128/128`. | Do not promote no-suffix full capture as the launcher default. Next work is C1 long-generation divergence localization: isolate whether the failure is caused by generation length, multiple full-slot warmup batches, prompt-cache/coalescing state, or full-capture cache-hit replay state. |
| 74 | C1 full-capture divergence localization | Early continuation replay-state blocker | Sprint 571 ran a diagnostic matrix under `/workspace/s571-full-capture-localize-artifacts`. Recreated `s569-shape` (`32` tokens, one warmup, one measured batch) diverged for `32/32` measured responses at continuation offset `1`, despite matching the representative first selected token `32974`. `s570-prompt-32` diverged `32/32` at offset `0`. `s570-prompt-64` diverged `15/32`, with first diffs at offsets including `0` and `7`. All completed cells had active graph replay and zero peer-copy/SYS hits. | Treat Sprint 569 as a positive but insufficient, non-reproduced signal. The next C1 target is early continuation replay-state repair around step `0 -> 1`, including prompt-cache/coalescing metadata, decode input token, selected-token handoff, slot assignment, and HC/current rebase timing. Avoid timing-shifted tensor comparisons; use request-level or same-logical-point artifacts. |
| 75 | C1 cache-miss capture-as-result repair attempt | Rejected; no production code change | Sprint 572 tested the narrow cache-miss hypothesis in `/workspace/s572-early-continuation-repair`: instead of serving eager and then capturing on cache miss, the candidate captured, instantiated, launched the captured full graph once, and mirrored the no-suffix full-capture host swaps after launch. The remote appliance build passed, but request-level serving parity still failed. `s569-shape` diverged for `32/32` measured responses with first diffs mostly at continuation offset `2`, and `s570-prompt-32` diverged for `32/32` measured responses at offset `0`. | Reject the cache-miss flow change and keep the promoted tree unchanged. Next C1 work is same-logical-point continuation instrumentation around step `0 -> 1`: prompt-cache/coalescing metadata, slot assignment/request order, decode input token, selected-token handoff, and full-capture HC/current rebase timing. Do not use timing-shifted tensor or scratch-buffer comparisons as proof. |
| 76 | C1 determinism baseline and divergence relocation | Diagnostic; methodology corrected, repair retargeted | Sprint 573 ran the determinism baseline prior gates lacked: `eager`, `control-A`, `control-B` (identical promoted config), and `full` from one binary on the Sprint 569 shape. The decode path is nondeterministic at the first token (`eager` produced `6` distinct continuations for `32` identical prompts; `control-A` vs `control-B` differed on `3/32`, all offset `0`). Promoted suffix-control matched eager within that floor (`3/32`). Full capture diverged `12/32` against eager, but its offset-`0` mismatches `{1,3,16}` are exactly noise requests; the real signal is a clean cluster `{8,9,10,11,17,18,19}` at offset `28`, which appears in no noise comparison (noise-baseline max offset is `0`). The cache-discriminator cell was confounded by the forced `position 250000` dominating output and is inconclusive. | Keep the promoted tree unchanged. Two durable corrections: full-capture gates must carry an identical-config determinism floor and judge against it and `eager`, not exact equality with one control run; and the C1 bug is late-position compressed-KV emit-replay state (offset `28` follows the ratio-4 emit at offset `27`), not the early-continuation step `0 -> 1` that 571/572 chased. Sprint 574: instrument the comp-emit boundary at offset `27 -> 28` then repair. |
| 77 | C1 full-capture divergence is position-dependent | Diagnostic; comp-emit disproved, layer not yet localized | Sprint 574 ran `eager` vs `full` at two positions. At `250000` full capture is benign (`12/32`, `7` real at offset `28`); at `250064` it breaks at offset `1` for `32/32` requests, producing `32` distinct sequences for identical prompts. So `250000` (used by every gate since Sprint 569) is an unusually clean position; the captured graph holds a position-derived value only correct near the capture position. Compressed-KV emit is off on this served path (the offset-`28` ratio-4 alignment was coincidence). RoPE and raw-window row selection read the live device position and are ruled out. `--decode-stage-checksum-gate` is misaligned with serving (one token per call -> every token logs `step 0`), so the layer is not yet localized. | Keep the promoted tree unchanged. No blind repair. Sprint 575 localizes with a multi-step capture/replay probe (`decode_steps > 1`) at `250064` plus stage checksums, diffing eager-step-k vs full-replay-step-k per `(layer, stage)`, then repairs and validates against the determinism floor at `250064` and `250000`. |
| 78 | C1 full-capture replay is per-slot correct | Resolved; blocker was a nondeterministic-oracle artifact | Sprint 575: single-slot testing (`SLOTS=8`, one active request) shows full-capture replay is bit-exact with eager -- identical generated sequences at `250000` (32 tokens) and `250064` (4 tokens). Sprint 574's catastrophic-`250064` claim is retracted: with the floor, `control-A` vs `control-B` (identical config) also diverges `32/32` there (`control-B` collapsed to `5` distinct sequences), so the position is measurement-unstable. The Sprint 573 offset-`28` cluster at `250000` is likewise a concurrent-batch effect (single-slot generates 32 tokens with no divergence). So the C1 full-capture replay mechanism is correct; every divergence since Sprint 570 is batch/concurrency nondeterminism (reduction order across slots, present in identical-config controls), and the blocker was an exact-equality oracle on a nondeterministic serving path. | Promotion is now a nondeterminism question, not correctness. Gate on per-slot single-request bit-exactness (passes today); characterize whether full capture amplifies concurrent-batch variance vs the `control-A`/`control-B` floor at matched concurrency. If within the floor, promote full capture for the `1.25-1.48x` decode win; otherwise fix the variance amplification as a separate, smaller issue. |
| 79 | C1 full-capture is batch-unstable (real defect) | Partly retracts 575; not promotable as-is | Sprint 576 measured the divergence in logit space (new `tp_ep_decode_top1_logit` diagnostic) with warmup disabled so all legs share positions. Identical eager runs are bit-identical on matched tokens (logit Δ = 0; `7/32` discrete router-tie flips), but two identical full-capture runs diverge on `32/32` slots with continuous logit Δ up to `3.63` (`p50 0.26`). So the inherent serving noise is tiny and discrete (MoE router ties / `router.cuh` atomics), and full capture adds large run-to-run batch instability that eager lacks. Full capture is per-slot bit-exact (single slot, Sprint 575) but batch-unstable under concurrency. The Sprints 570-574 divergences were this instability conflated with the small eager floor. | Full capture is a real defect, not tolerated noise; not promotable as-is. Sprint 577 localizes the instability (EP-compose `atomicAdd`, non-reset route/compose scratch, or HC ping-pong) via the logit diagnostic + a deterministic-reduction probe; confirm by collapsing the full-vs-full floor toward the eager floor. Judge in logit space, not token equality. |
| 80 | C1 instability localized to graphed route/compose accumulation | Localized; sanitizer ruled out; rebuild next | Sprint 577: the full-vs-full logit floor scales with active routed tokens (1-2 active ~bit-exact; `8` -> Δ 1.21, `8/8` flips; `32` -> Δ 3.63, `32/32`). The 8-slot graph structure is constant across these, so it is not a static pointer/buffer bug -- it tracks active route count, i.e. accumulation-order nondeterminism in the graphed route/compose, exposed by graph replay (eager uses the same atomics and is bit-stable). A `compute-sanitizer --tool initcheck` attempt initialized NCCL/8-GPU but OOMed during expert load (shadow memory + ~24 GB model > 32 GB) before reaching decode (`0 errors` in the load path); no smoke reproduces the graph-replay path, so the tool cannot reach this bug. | Sanitizer is not viable here. Sprint 578: deterministic-compose rebuild (`nccl_reduce_scatter_compose_gate` or a non-atomic fixed-order combine; compile-time, needs rebuild). Rerun full-vs-full at `8`/`32`; if the floor collapses toward eager, the determinism defect is confirmed and fixed, and full capture can be re-evaluated for promotion. |
| 81 | C1 full-capture batch-instability FIXED | Fixed; correctness-preserving | Sprint 579: per-stage runtime diff (matched positions via a dummy leg) localized the first divergence to the captured `compressed_kv` stage's `attn_q_b.d_out` on `dense_stream` at the first ratio-4 emit position. Root cause: `enqueue_rank_streams_wait_after_dense_streams` (`output_head.cu`) was a dense->rank-only barrier while the eager path it replaces fully drains both streams; the one-way edge let `dense_stream` lap `rank_stream` across replays and race the in-place writes to `attn_q_b.d_out`. Made the barrier bidirectional -> full-vs-full generated-sequence mismatch `8/8 -> 0/8` (two identical full-capture runs now bit-identical). Ordering-only, correctness-preserving, in the shared cudagraph helper. | Determinism defect fixed; full capture is now run-to-run deterministic. Next: standard serving parity/perf promotion gate vs the eager floor, re-confirm suffix-control under the strengthened barrier, then decide the launcher default. The `1.25-1.48x` decode win is now on a sound path. |
| 82 | C1 full-capture promoted to launcher default | Promoted; C1 complete | Sprint 580: the serving promotion gate passed. At `32` slots / pos `250000`, parity was perfect within the determinism floor (floor `0`, full-vs-full `0`, eager-vs-full `0`) and no-suffix full capture was `1.203x` wall / `1.518x` decode faster than suffix-control (median latency `42.09s -> 34.98s`). Flipped the TP/EP launcher to a 3-mode `DS4_V100_TP_EP_DECODE_GRAPH_MODE` (`full` default; `suffix`/`eager` opt-outs; `GRAPH_SUFFIX_REPLAY` back-compat). | C1 is complete: both suffix-replay and the now-default full-capture graph paths are correct and promoted. Next ordered item: the tuning sprint (reference-shape perf, shape envelope, NCCL pinning, C4 spill), then the MTP bet. |
| 83 | Tuning baseline + decode-domain gap attribution | Baseline set; EP is the headroom | Sprint 581: new reference baseline `26.8` tok/s agg decode at 32 slots / 256K / 64 tok/req on the full-capture default (`1.225x` over suffix; `2.34` vs `1.53` per-req decode; latency `92.1s -> 74.9s`). Gap attribution (eager, `14.4` ms/step): EP/MoE all-to-all `65.2%` (the utilization waves; ~0.75 tok/expert -> near-empty grouped-GEMM tiles, SMOCC ~0.08-0.11), attention ~12%, compose ~11%, HC ~8%, host-sync orchestration (PCIe/GPU0 skew) only ~5%. | Throughput headroom is EP, not host-sync. Next: MTP/B1 (fills expert tiles -> attacks the 65% at its root) with B2 (dispatch/combine fusion) alongside. Shape-envelope/NCCL-pinning/C4-spill deferred (situational). |
...
- Sprint 597 deferred items (B2 stage menu, `_device_total_tokens` ABI,
  peer-copy accounting for graph remote-copy kernels, NCCL ring pinning,
  envelope sweep + C4 KV spill): see `docs/sprints/SPRINT-597-DEFERRED.md`.
...
| 2026-05-29 | Sprint 549 retired rejected post-attention padding knobs. | Removed the inactive/default-off `device_actual_route_sync`, static rank/executor/compose cap, and masked compact-copy branches from active code, including the unused masked-copy CUDA kernel and scaffold/profile fields. The promoted fixed-capacity route plan remains the only supported graph-stable padding surface. Remote V100 appliance build passed in `/workspace/s549-padding-cleanup`. | Treat this as cleanup only, not a performance claim. Do not revive static caps or masked compact copy as C1 levers. Next C1 work should implement real full-shape device-masked executor/compose efficiency or the typed-KV/runtime device-state refactor for full capture. |
| 2026-05-29 | Sprint 550 reduced compact EP pack padding work. | Changed `ep_pack_route_dest_shards_kernel` from a flat `routes * hidden` launch to one block per graph-visible route. Active routes write the same packed values; inactive padded routes return before hidden-wide work. Remote build passed in `/workspace/s550-ep-pack-route-block`; short selected-token graph sanity passed with `http_200=8`, first token `29361`, `graph_audit_blocker=none`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. A warmed `32` request / `32` slot / `256K` / `64` token graph candidate matched Sprint 540's first token `107027`, preserved `43/43` cache hits/replays with zero position invalidations and zero peer/SYS transport, but was performance-neutral/slightly slower versus the Sprint 540 graph control: request window `90.181067s -> 90.528551s`, client generated tok/s `22.709904 -> 22.622697`, scaffold ms/token `666.058962 -> 668.605160`. | Promote the route-block compact EP pack kernel as structural cleanup, not as a measured throughput win. The obvious compact-pack padding site is not the dominant residual steady-state cost; continue C1 with typed-KV/full-capture device-state work or a larger executor/compose shape design with direct evidence. |
| 2026-05-29 | Sprint 551 made raw typed-KV graph stores/loads decode-position dynamic. | Added TP runtime row APIs whose kernels read `RankState::d_decode_position` and compute the physical KV row on device, then wired graph-mode raw typed-KV store/load to them. The appliance and TP runtime smoke built in `/workspace/s551-typed-raw-position`; promoted graph sanity stayed clean at `8x4` with first token `29361`, `graph_audit_blocker=none`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. A targeted smoke compared dynamic-position `attn_raw` against the existing static-position row path at position `262081` and matched exactly: `physical_row=65`, `bad_values=0`, `max_abs=0.000000000`. | Promote as C1 capture-readiness cleanup, not a performance claim. Full capture remains position-keyed; next C1 work is emitted compressed/indexer typed-KV device state, including host `emitted`, row counters, row-position arrays, and static typed-KV runtime calls. |
| 2026-05-29 | Sprint 552 made emitted compressed/indexer typed-KV graph stores/loads decode-position dynamic. | Graph-mode emitted compressed attention and indexer typed-KV store/load now use the dynamic-position TP runtime APIs from Sprint 551, passing `RankState::d_decode_position` instead of host `opt.position` to select the physical KV row. The appliance and TP runtime smoke built in `/workspace/s552-emitted-typed-position`; targeted smokes matched static row behavior exactly for emitted `attn` at `physical_row=65648` and `indexer` at `physical_row=65520`, both with `bad_values=0` and `max_abs=0.000000000`. Promoted graph sanity stayed clean at `8x4` with first token `29361`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. | Promote as C1 capture-readiness cleanup, not a performance claim. Full capture remains position-keyed; next C1 work is emitted-row topology and bounded-row device state, including host `emitted`, bounded-row source/dest pointers, row counters, and row-position arrays. |
| 2026-05-29 | Sprint 553 made emitted typed-KV bounded-row selection decode-position dynamic. | Added bounded dynamic-position TP runtime APIs whose kernels compute both physical KV row and compact bounded row from `RankState::d_decode_position`, then wired graph-mode emitted compressed attention and indexer typed-KV store/load to pass base compact-row buffers instead of host-offset bounded-row pointers. The appliance and TP runtime smoke built in `/workspace/s553-bounded-typed-position`; targeted smokes at emitted position `262087` matched static row behavior exactly for compressed `attn` at `physical_row=65649`, `bounded_row=1`, and `indexer` at `physical_row=65521`, `bounded_row=1`, both with `bad_values=0` and `max_abs=0.000000000`. Promoted graph sanity stayed clean at `8x4` with first token `29361`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. | Promote as C1 capture-readiness cleanup, not a performance claim. Full capture remains position-keyed; next C1 work is emitted topology and row-position metadata, including host `emitted`, row counters, row-position arrays, and typed-history reload. |
| 2026-05-29 | Sprint 554 made graph-mode typed-history row loads decode-position dynamic. | Added a TP runtime history-row load API that computes the historical source position from `RankState::d_decode_position`, compression ratio, row kind, and bounded row inside the CUDA kernel, then wired graph-mode compressed attention and indexer typed-history reloads to pass base compact-row buffers. The appliance and TP runtime smoke built in `/workspace/s554-history-row-position`; targeted smokes at current position `262095` / history row `1` matched the static-position row load exactly for compressed `attn` (`history_position=262087`, `physical_row=65649`) and `indexer` (`physical_row=65521`), both with `bad_values=0` and `max_abs=0.000000000`. Promoted graph sanity stayed clean at `8x4` with first token `29361`, `43/43` replays, zero position invalidations, zero peer-copy/SYS, zero NCCL SYS edges, and zero VRAM failures. | Promote as C1 capture-readiness cleanup, not a performance claim. Full capture remains position-keyed; next C1 work is emitted/non-emitted topology stability plus remaining host emitted-row counters/arrays, or an explicit saturated-history full-capture boundary. |
| 2026-05-29 | Sprint 555 rejected served full-capture position-key removal. | Tested no-suffix full-capture persistent replay with compressed KV off. The cache-key change delivered the structural signal (`43/43` persistent replays, zero position invalidations, zero peer/SYS transport), but selected-token correctness failed against same-shape eager control: eager first token was `29361`, while candidates returned `128819` and then `118235` after prefix-helper and position-copy-barrier fixes. Candidate code was removed. | Keep the promoted `compose_eager_final_hc` graph suffix replay as the serving graph path. The full-capture position key is still a correctness guard; next C1 work must localize the first divergent stage/checksum before another cache-key relaxation. |
| 2026-05-29 | Sprint 556 localized the full-capture replay-probe failure. | Exact-shape `8` request / `8` slot / `256K` / `2` token validation showed eager first token `128819`; no-suffix full capture without replay-probe also returned `128819` with `43/43` captures and no peer/SYS transport; adding immediate replay-probe returned `118235` with `43/43` replays. The code path explains the split: replay-probe captures by executing the full step on live buffers, then immediately launches the captured full graph against already-advanced buffers. That is not a valid no-suffix full-capture parity test. | Do not use no-suffix replay-probe output as a full-capture parity signal until the probe snapshots/restores device input state or splits cache-miss behavior so capture execution is kept and replay is tested only from fresh state. Next C1 work is full-capture replay validation repair, not another cache-key relaxation. |
| 2026-05-29 | Sprint 557 guarded invalid no-suffix replay-probe. | `engine/decode_loop.cu` now rejects `--decode-cudagraph-replay-probe-gate` when no suffix stage is selected, logging `full_capture_live_state_replay_requires_snapshot` and returning `cudaErrorNotSupported` instead of returning a token from a double-applied full step. Remote build passed. The blocked no-suffix probe returned `http_200=0` with the expected log line; a promoted suffix sanity still passed with `http_200=1`, `43/43` captures, `43/43` replays, zero persistent invalidations, and zero peer/SYS transport. | Treat this as validation-harness repair, not a throughput change. The next C1 step is a real fresh-state no-suffix replay validation mechanism before any further full-capture cache-key relaxation. |
| 2026-05-29 | Sprint 558 repaired fresh-state full-capture replay validation. | No-suffix replay-probe now splits cache miss from cache hit: on cache miss the served request runs eager and the full graph is captured/instantiated only as audit/cache state; a later same-position request can replay the cached graph from fresh request buffers. Remote build passed in `/workspace/s558-fresh-state-replay`. Eager sequential control returned selected tokens `128819`, `128819`; full-capture fresh-state replay returned the same selected tokens and reported `scaffold_decode_cudagraph_persistent_cache_hits=43`, `scaffold_decode_cudagraph_replay_attempted=43`, `scaffold_decode_cudagraph_replay_succeeded=43`, zero invalidations, zero peer-copy/SYS, and zero NCCL graph SYS edges. Promoted suffix sanity still passed. | Promote only as validation-harness repair. The second replay checksum drifted versus eager (`5174931161` -> `5002850195`), so full capture remains diagnostic-only and position-keyed. Next ordered work is localizing the checksum drift before another full-capture cache-key or serving-default change. |
| 2026-05-29 | Sprint 559 fixed full-capture replay checksum drift. | Stage checksums showed `compose.next_hidden` matched while `final_hc_shard` was stale after no-suffix full-capture cache-hit replay. Root cause: the CUDA graph captured final-HC kernels but not the host-side final-HC/scratch pointer swap that eager performs after enqueue. `engine/decode_loop.cu` now mirrors that swap after successful no-suffix persistent full-capture replay, guarded by `final_hc_carry_gate`, `tp_hc_final_expand_gate`, and no suffix stage. Claude bug-find review caught the missing `tp_hc_final_expand_gate` guard before final validation. Remote build passed in `/workspace/s559-checksum-drift`; full-capture replay now matches eager selected tokens and checksums for both sequential requests (`7238127778`, `5174931161`) with `43/43` cache-hit replays, zero invalidations, zero peer/SYS transport, and promoted suffix sanity still passes. | Promote as full-capture validation/correctness repair only. No-suffix full capture remains diagnostic-only and position-keyed; next ordered C1 work is emitted topology and row-position metadata before retrying cross-position cache-key relaxation. |
| 2026-05-29 | Sprint 560 mirrored emitted-row host metadata after full-capture replay. | Added a no-suffix full-capture cache-hit replay helper that mirrors eager compressed attention/indexer emitted-row host metadata: row counters, row-position arrays, loaded flags, and loaded positions. The served appliance still defaults compressed KV off, so this was validated as C1 readiness/no-regression rather than compressed-KV serving promotion. Build passed in `/workspace/s560-emitted-metadata`; served-default validation confirmed `compressed_kv_layers=0` and no-regression with `43/43` full-capture cache-hit replays, zero invalidations, zero peer/SYS, and zero NCCL graph SYS edges. A one-off compressed-KV entrypoint then exercised the branch without adding a permanent flag: compressed-KV eager and no-suffix full-capture replay both saw `compressed_kv_layers=86` / `compressed_kv_emitted_layers=42` and matched selected tokens/checksums exactly (`58204`, `109597`; `7265791446`, `79399742586`) with `43/43` cached graph replays on the replay leg. Claude bug-find review confirmed the helper matches eager happy-path loaded-row algebra. | Promote as C1 readiness/correctness repair only. Full capture remains diagnostic-only and position-keyed. Next C1 work is emitted/non-emitted topology graph-stability without adding a permanent flag matrix. |
| 2026-05-29 | Sprint 561 made compressed-KV emitted topology graph-stable. | Graph-mode compressed-KV now enqueues the emitted-row topology consistently and uses `d_decode_position` device guards so emitted-row kernels/copies no-op on non-emitted positions. Eager keeps the compact host-selected topology, and host row metadata still advances only for real emitted positions. Remote build passed in `/workspace/s561-emitted-topology`; served-default compressed-KV-off sanity passed with `compressed_kv_layers=0`, `43/43` replay hits, zero invalidations, zero peer/SYS, and zero NCCL graph SYS. The one-off compressed-KV binary matched eager vs no-suffix full-capture replay at emitted position `262083` (`58204`, `109597`; `7265791446`, `79399742586`) and adjacent non-emitted position `262084` (`109939`, `107875`; `561376577`, `2841198172`). Claude bug-find identified graph-mode typed-KV extra non-emitted work as the main risk; the non-emitted parity gate covered that exact surface. | Promote as C1 readiness/correctness repair only. Full capture remains diagnostic-only and position-keyed. Next ordered C1 work is cross-position cache-key relaxation as a diagnostic parity sprint, not serving promotion. |
| 2026-05-29 | Sprint 562 rejected full-capture cross-position cache-key relaxation. | Retried the no-suffix full-capture cache-key relaxation by removing the position key from persistent invalidation and replay-probe cache-hit detection. Build passed in `/workspace/s562-cross-position`. The two-request same-session probe matched eager and proved the candidate was actually reusing across positions (`position=262084`, `cached_position=262083`, `43` cache hits, zero invalidations). Claude bug-find warned that hidden position-derived graph arguments could still exist, so the validation was extended to six same-session requests. That stronger probe failed on request three: eager token/checksum `117465` / `17092309830`, replay token/checksum `2039` / `110810249310`; later requests diverged as well. The candidate code was removed. Promoted suffix/default sanity still passed with `http_200=2`, `43/43` replay hits, zero invalidations, zero peer/SYS, zero NCCL graph SYS, and `compressed_kv_layers=0`. | Reject. Full capture remains position-keyed. Next C1 sprint should localize the residual captured-position state before another cache-key retry; do not add a flag or attempt promotion around the failed relaxation. |
| 2026-05-29 | Sprint 563 localized the cross-position replay divergence to layer 1 HC-current. | Used a temporary remote-only relaxed build at `/workspace/s563-localize` with `--decode-stage-checksum-gate`. The three-request same-session diagnostic reproduced the relaxed replay divergence. Stage checksum comparison showed occurrence 0 matched completely; occurrence 1 had no layer-0 diffs and first diverged at layer 1 `hc_current` across `current_shard`, `current_full`, `current_full_rank_major`, and `final_hc_shard`. | Full capture remains position-keyed. Next C1 work is repairing or further localizing the inter-layer current/HC state handoff after layer-0 full-capture cache-hit replay. |
| 2026-05-29 | Sprint 564 added a full-capture final-HC pointer cache key and rejected the cache-miss double-advance hypothesis. | A capture-as-result candidate failed immediately: stream capture did not materialize live tensors, returning selected token `0` / checksum `5074584678` instead of eager token `24426` / checksum `128829740021`, so cache misses must continue serving eager. The promoted code records `final_hc_shard_key` in `TpCudaGraphLayerExec` and includes it in no-suffix full-capture cache-hit/invalidation checks. Remote build passed in `/workspace/s564-cache-miss-state`; position-keyed no-suffix same-session diagnostic matched eager for three requests (`24426`, `2039`, `117465` with checksums `128829740021`, `106648190597`, `17092309830`). A remote-only relaxed-position retry with the pointer key still failed on request 3 (`117465` / `17092309830` eager vs `128818` / `81184816026` replay). Promoted suffix sanity passed with `http_200=2`, `43/43` suffix replay hits on the second request, zero invalidations, zero peer/SYS, zero NCCL graph SYS, and `compressed_kv_layers=0`. | Promote the final-HC pointer key as a defensive guard, but do not relax the full-capture position key. The remaining C1 blocker is captured position-dependent state: audit scalar `opt.position` and derived row arguments in replay-critical kernels and move them to the `d_decode_position` device path before another cross-position retry. |
| 2026-05-29 | Sprint 565 localized the post-pointer-key relaxed replay divergence to rank-major current. | Rebuilt a remote-only relaxed-position diagnostic after Sprint 564 and ran the three-request same-session selected-token shape with `--decode-stage-checksum-gate`; artifacts are under `/workspace/s565-stage-artifacts`. Response-level behavior reproduced the failure: request 3 eager `117465` / `17092309830`, relaxed replay `128818` / `81184816026`. Stage checksum parsing showed occurrence 0 matched, and occurrence 1 first sequentially differed at layer 0 `hc_current.current_full_rank_major` rank 0 (`1673392381` eager vs `2096617547` relaxed replay), while the immediately preceding `current_shard` and `current_full` rank-0 checksums matched. | No production code change. Full capture remains position-keyed. Next C1 work is the rank-major current replay path: explain or repair why `d_current_full_rank_major` diverges under graph replay before any further cache-key relaxation. |
| 2026-05-29 | Sprint 566 repaired the C1 replay diagnostic and moved the blocker to route replay state. | Added an order-sensitive diagnostic checksum kernel and comparable eager/replay `step_snapshot` records under the existing `--decode-stage-checksum-gate`. Remote clean production build passed in `/workspace/s566-rank-major-snapshot`. A remote-only relaxed-position diagnostic under `/workspace/s566-stage-artifacts` reproduced the response failure on request 3 (`117465` / `17092309830` eager vs `128818` / `81184816026` replay). The new comparable snapshots showed occurrence 0 matched, occurrence 1 matched token/checksum and all end-of-step tensors except `route_a` across all layers/ranks, and occurrence 2 had only layer-0 `route_a` drift before layer-1 and later tensors diverged broadly. | Treat the Sprint 565 rank-major HC-current diff as a timing artifact. Full capture remains position-keyed. Next C1 work is route-plan/route-pack replay boundary repair before any further no-suffix cache-key relaxation. |
| 2026-05-29 | Sprint 567 cleared route metadata as the semantic C1 blocker. | Added `route_totals`, `route_slots`, and `route_weights` to the diagnostic `step_snapshot`. Remote clean production build passed in `/workspace/s567-route-replay`; a temporary relaxed-position diagnostic reproduced the request-3 failure. Because eager-vs-graph route metadata differs by planner mode, a graph-vs-graph control was run with position-keyed graph under `/workspace/s567-route-control` and relaxed graph under `/workspace/s567-route-replay`; artifacts are under `/workspace/s567-graph-compare-artifacts`. Occurrence 0 matched completely. Occurrence 1 differed only in full `route_a` scratch while route totals/slots/weights, output tensors, selected token, and checksum still matched. Occurrence 2 had only layer-0 `route_a` scratch drift before layer-1 current/output tensors diverged broadly. | Treat full-buffer `route_a` drift as scratch noise, not the semantic blocker. Full capture remains position-keyed. Next C1 work returns to the inter-layer current/HC pointer-buffer handoff, especially captured graph pointer arguments versus eager/replay host swaps of `d_final_hc_shard` and `d_hc_scratch_shard`. |
| 2026-05-29 | Sprint 568 fixed no-suffix full-capture cross-position replay. | Stored captured final-HC input/output buffer addresses per rank and added a no-suffix full-capture replay rebase that copies the live logical HC state into the graph's captured input buffer before launch, then lets the existing post-replay host swap point at the captured output buffer. Remote production build passed in `/workspace/s568-hc-rebase-promoted`. A pre-promotion graph-vs-graph check passed at three requests in `/workspace/s568-graph-compare-artifacts`, and the stronger six-request graph-vs-graph check passed in `/workspace/s568-graph-compare-6-artifacts`. The promoted tree then passed six-request eager-vs-full-graph parity in `/workspace/s568-promoted-6-artifacts`: tokens/checksums `24426/128829740021`, `2039/106648190597`, `117465/17092309830`, `61356/19814694371`, `25681/110664132098`, `115959/18767001400`; full graph reported `43` captures, `215` persistent cache-hit replays, and zero invalidations. | Promote as opt-in C1 correctness/readiness repair. No-suffix full capture no longer needs the position key when HC rebase is available, but full capture is still not a serving default. Next C1 work is serving parity/performance metrology with deterministic generation, warmup, startup/init excluded, and a long prompt. |
| 2026-05-29 | Sprint 569 produced a positive no-suffix full-capture serving-metrology signal. | Remote build passed in `/workspace/s569-serving-metrology`. The serving A/B used `32` slots / `256K`, deterministic generation (`temperature=0`, `top_p=1`), a long prompt, one full-slot warmup batch, and one full-slot measured batch with startup/readiness and warmup excluded. Promoted suffix-control and opt-in no-suffix full capture matched all `32/32` measured generated token sequences by index and as a multiset. No-suffix full capture improved request-window generated throughput `12.603435 -> 16.807308` tok/s, continuation throughput `12.209578 -> 16.282079` tok/s, decode generated throughput `1.506832 -> 2.288130` tok/s, and median latency `81.205441s -> 60.873505s`. | Treat this as a strong opt-in C1 signal, not a default flip yet. Repeat with a longer steady-state measured window and better graph/status counter extraction before promoting no-suffix full capture as the launcher default. |
| 2026-05-29 | Sprint 570 rejected no-suffix full-capture default promotion. | Remote build passed in `/workspace/s570-full-capture-promotion`. The longer serving gate used `32` slots / `256K`, deterministic generation, `64` tokens/request, `64` warmup requests excluded, and `128` measured requests. Both legs returned `128/128` HTTP 200 and replayed graphs in the measured window with zero peer-copy/SYS hits. The candidate remained faster (`16.618822 -> 20.814267` continuation tok/s wall; median latency `132.083580s -> 103.958728s`) but failed parity: generated token sequences and generated text diverged for all `128/128` measured responses. | Reject the default flip. Continue C1 by localizing the long-generation divergence before another promotion gate. |
| 2026-05-29 | Sprint 571 localized no-suffix full-capture divergence to early continuation replay state. | The recreated Sprint 569 shape did not reproduce the original pass: it returned `32/32` HTTP 200 on both legs, but generated sequences diverged for `32/32` measured responses at continuation offset `1`. With the Sprint 570 prompt and `32` tokens, generated sequences diverged `32/32` at offset `0`. With the Sprint 570 prompt and `64` tokens, generated sequences diverged `15/32`. All completed cells had graph replay active and zero peer-copy/SYS hits. | No promotion. Next sprint should instrument/repair comparable step `0 -> 1` request state: prompt-cache/coalescing, selected-token/decode-input handoff, slot assignment, and full-capture HC/current rebase timing. |
| 2026-05-29 | Sprint 572 rejected cache-miss capture-as-result as the early-continuation fix. | The candidate was built remotely in `/workspace/s572-early-continuation-repair` and validated through deterministic request-level generated sequences. `s569-shape` still diverged for `32/32` measured responses, now mostly at continuation offset `2`; `s570-prompt-32` still diverged for `32/32` measured responses at offset `0`. The local code trial was removed. | Keep the promoted tree unchanged. Next C1 sprint should add same-logical-point instrumentation for prompt/cache/coalescing, slot order, selected-token/decode-input handoff, and HC/current rebase timing; avoid timing-shifted tensor comparisons. |
| 2026-05-29 | Sprint 573 established a determinism floor and relocated the C1 full-capture bug. | Four legs from one binary on the Sprint 569 shape. Decode is nondeterministic at the first token (`eager` gave `6` distinct continuations for `32` identical prompts; `control-A` vs `control-B` differed `3/32`, all offset `0`). Promoted suffix-control matched eager within that floor. Full capture diverged `12/32` against eager, but offset-`0` mismatches were the noise requests; the real signal is `{8,9,10,11,17,18,19}` at offset `28` (no noise comparison reaches past offset `0`). The cache-discriminator cell was confounded by the forced `position 250000` and is inconclusive. | Keep the promoted tree unchanged. Full-capture gates must carry an identical-config determinism floor; the C1 bug is late-position compressed-KV emit-replay state (offset `28` follows the ratio-4 emit at offset `27`), not early continuation. Sprint 574 instruments the comp-emit boundary then repairs. |
| 2026-05-29 | Sprint 574 found the C1 full-capture divergence is position-dependent and disproved comp-emit. | `eager` vs `full` at `250000` is benign (`12/32`, `7` real at offset `28`); at `250064` it breaks at offset `1` for `32/32` requests (`32` distinct sequences for identical prompts). Compressed-KV emit is off on the served path, so the offset-`28` ratio-4 alignment was coincidence. RoPE and raw-window row selection are device-dynamic and ruled out. `--decode-stage-checksum-gate` is misaligned with serving (every token logs `step 0`), so the layer is not yet localized. | Keep the promoted tree unchanged; no blind repair. `250000` understates the bug. Sprint 575 localizes with a multi-step capture/replay probe at `250064` plus stage checksums, then repairs against the determinism floor. |
| 2026-05-29 | Sprint 575 resolved C1: full-capture replay is per-slot correct; the blocker was a nondeterministic-oracle artifact. | Single-slot (`SLOTS=8`, one request) full-capture replay is bit-exact with eager at `250000` (32 tok) and `250064` (4 tok). Sprint 574's catastrophic-`250064` is retracted: the `250064` floor `control-A` vs `control-B` is `32/32` (`control-B` collapsed to `5` distinct sequences), so the position is measurement-unstable. The `250000` offset-`28` cluster is a concurrent-batch effect. Every divergence since Sprint 570 is batch nondeterminism (reduction order across slots), present in identical-config controls. | Promotion is a nondeterminism question now. Gate on per-slot single-request bit-exactness (passes); characterize concurrent-batch variance of full vs the control floor; promote for the `1.25-1.48x` decode win if within the floor. |
| 2026-05-29 | Sprint 576 measured the divergence in logit space: full capture is batch-unstable, a real defect (partly retracts 575). | New `tp_ep_decode_top1_logit` diagnostic; warmup disabled so all legs share positions. Eager-vs-eager is bit-identical on matched tokens (logit Δ = 0; `7/32` discrete router-tie flips); full-vs-full diverges `32/32` with logit Δ up to `3.63` (`p50 0.26`); full-vs-eager sits between (`1.5`, `19` flips). Inherent serving noise is tiny/discrete (MoE router ties); full capture adds large run-to-run batch instability eager lacks. Single-slot full is bit-exact, so it is a concurrency effect. | Full capture is a real defect, not tolerated noise; not promotable as-is. Sprint 577 localizes the instability (EP-compose `atomicAdd` / non-reset route scratch / HC ping-pong) via the logit diagnostic + deterministic-reduction probe; confirm by collapsing the full-vs-full floor toward the eager floor. |
| 2026-05-29 | Sprint 577 localized the instability to the graphed route/compose accumulation (scales with active routes); compute-sanitizer ruled out. | Full-vs-full logit floor: 1-2 active ~bit-exact, `8` -> Δ 1.21 (`8/8` flips), `32` -> Δ 3.63 (`32/32`). The 8-slot graph is constant across these, so not a static pointer/buffer bug -- it tracks active route count (accumulation-order nondeterminism under graph replay; eager uses the same atomics and is bit-stable). `compute-sanitizer initcheck` initialized NCCL/8-GPU but OOMed during expert load before decode (`0 errors` in load path); no smoke reproduces the graph-replay path. | Sanitizer not viable. Sprint 578: deterministic-compose rebuild; rerun full-vs-full and confirm the floor collapses toward eager. |

| 2026-05-29 | Sprint 578 narrowed the C1 instability mechanism. | Ruled out compose accumulation (default uses deterministic `ep_pack`, not the `atomicAdd` kernel) and route ordering (deterministic by construction). Live hypothesis: a missing intra-graph event dependency in the captured compose -- eager serializes phases with `cudaStreamSynchronize` (illegal during capture), so a captured race fits all observations (graph-replay-specific, scales with active routes, eager bit-stable). | Not yet fixed. Trace the captured-region ordering with `nsys` (lighter than compute-sanitizer; may run on the full appliance), add the missing event dependency or a deterministic combine, rebuild, and confirm the full-vs-full floor collapses toward eager. |

| 2026-05-29 | Sprint 579 FIXED the C1 full-capture batch-instability. | Runtime per-stage diff localized the first divergence to the captured `compressed_kv` stage's `attn_q_b.d_out` on `dense_stream`; root cause was `enqueue_rank_streams_wait_after_dense_streams` being a dense->rank-only barrier (eager fully drains both streams), letting `dense_stream` lap `rank_stream` across replays. Made the barrier bidirectional; full-vs-full mismatch `8/8 -> 0/8`. Ordering-only fix. | Determinism defect fixed. Next: serving parity/perf promotion gate vs the eager floor; re-confirm suffix-control; then decide launcher default. |

| 2026-05-29 | Sprint 580 promoted no-suffix full capture to the TP/EP launcher default. | Gate at `32` slots / pos `250000`: parity perfect within the determinism floor (floor `0`, full-vs-full `0`, eager-vs-full `0`); full capture `1.203x` wall / `1.518x` decode faster than suffix-control (latency `42.09s -> 34.98s`). Launcher now uses `DS4_V100_TP_EP_DECODE_GRAPH_MODE` (`full` default; `suffix`/`eager` opt-outs). | C1 complete. Next: tuning sprint, then MTP. |
...
| 2026-05-30 | Sprint 584 RESULT: EP=8 MTP weight integration + forward body DONE + validated on the pod; specdec loop scoped as Sprint 585. | Built incrementally across ~16 validated commits (shipped 1.22x serving byte-intact throughout): the full EP=8 weight pack (32-family converter, fused gate_up, EP-split contract), all runtime loaders (expert/non-expert/HC-rank-dist/router-shards/norm-rank/dense-F8/output-head), the structural extensions (kLayers->44, ds4_layer_ratio/layer_ratio(43)=0, SharedHcControls/SharedDenseOps/KV arrays [44], 9-file guard migration), and the MTP forward BODY executing through the multi-rank EP decode (run_layer(43): decode_pass=1, decode_checksum=6684186189, expert_rows=16 EP-split). The structural unknown -- can the MTP layer be a participant in the appliance's 43-layer-specialized EP decode? -- is resolved: yes. | The ONE remaining piece is the TP/EP specdec loop (Phase B): a from-scratch multi-rank serve-path decode driver (draft K via the MTP forward -> verify K+1 via the existing ds4_replay_verify_token_block which has no callers yet -> accept/reject), integration point appliance/http_server.cu generation loop, validated by a live pod serving run (acceptance + throughput vs the 26.8 tok/s baseline). All components + the integration point are built/identified. Scoped as Sprint 585 (docs/sprints/SPRINT-585.md), to run as a fresh focused session per user decision (avoids degraded-context churn in the shared decode path). Sidecar deletion follows once the unified-pack draft matches the LP sidecar reference. |
| 2026-05-30 | Sprint 586 repaired MTP per-slot raw-window state but did not clear acceptance. | Added cache-slot-owned `mtp_raw_rows`, reset it on miss/eviction/prompt mismatch, threaded it into `run_token_major_serving_loop`, and made layer 43 use active-slot MTP raw counts for `true_ds4_attention_raw_valid_rows`. Pod build passed. The acceptance harness now shows `mtp_raw_valid_rows` growing from `1` upward across prefill/continuation, proving the HTTP one-step loop no longer pins the MTP raw window to one row. | B1 remains blocked: acceptance stayed `0/71` for both same-index and next-index comparisons. The next MTP sprint should stop guessing and add same-logical-point numerical localization for raw ring row contents, prologue output, attention output, FFN output, and MTP head logits before building the K-wide verifier. |
| 2026-05-30 | Sprint 584 MAJOR CORRECTION: the existing MTP code is LP-era, not EP=8 (user-flagged). | The appliance had an LP=8 (layer-parallel: each layer's 256 experts on one GPU, 43 layers spread across 8) first pass, then the current EP=8 (expert-parallel: 32 experts/rank of every layer, decode-time all-to-all = the measured "EP 65%"). The deployed s181 pack is LP (86 turbomind rows = 43 layers x 2 expert tensors, one owning_gpu each, unequal shard sizes). `engine/mtp_step.cu` + the sidecar are the LP MTP (Q4_K, SEPARATE gate/up, self-contained non-EP compute); the Sprint 582/583 converter inherited the LP framing. The EP=8 production runtime loads BOTH a `--contract` (ep_expert rows, 32/rank) AND a `--tm-index`; the contract slices the per-layer turbomind blobs into per-rank 32-expert sets at load (runtime_pack.cu ep_expert parse; runtime_types.cuh kLocalExperts=32). | Build a NEW EP=8 MTP as layer 43 of the unified EP model, reusing the per-layer EP routed-FFN dispatch (`ds4_gpu_arena_turbomind_mxfp4_routed_gate_up_swiglu_down_sum_f32`, FUSED gate_up mxfp4, EP-split). Phases: converter fused-gate_up -> EP pack emission (equal per-rank shards) -> runtime layer-43 bind -> EP MTPBlock.forward (reuse shared dispatch + MTP embedding-combine prologue) -> sidecar delete -> specdec loop. Full code-grounded plan in docs/sprints/SPRINT-584.md. Prior MTP records (582/583) are LP-era. |
| 2026-05-30 | Sprint 584 re-validation CORRECTED the Sprint 583 "weight-pack complete end-to-end" claim. | Inspecting the actual shard artifacts (not the success log) showed `gpu0.weights` = 3.58 GB (all 256 experts) and `gpu1-7.weights` = **0 bytes** -- the appliance-pack/turbomind-pack run was SINGLE-GPU, driven from the standalone `owning_gpu=0` pack-index, NOT the EP8 contract. The CONTRACT is correct (`mtp-contract2`: 24 ep_expert rows @ 32/rank, 72 dense_tp, 512 kv_shard, 64 replicated_control -- a valid TP8/EP8 plan), but the EP8-distributed 8-GPU shard emission was never executed/validated. Root cause: no contract -> per-rank pack-index bridge was run; the experts went through `turbomind-pack`'s per-`owning_gpu` output against a single-GPU index. | The true remaining weight-integration step (before runtime bind) is to drive appliance-pack/turbomind-pack from the EP8 contract -> 8 non-zero shards, validated by per-rank byte counts (~447 MB experts/rank + dense). Then runtime_pack.cu bind (loads each rank's non-empty shard), sidecar delete, MTPBlock.forward, specdec loop. Lesson: validate shard emission by inspecting per-rank artifact sizes, not the packer's exit status. |
| 2026-05-30 | Sprint 590 fixed output-head HC-control rank slicing but MTP acceptance is still blocked. | Same-activation instrumentation compared the MTP draft head against a CPU reference using the actual slot-0 layer-43 HC shards and `blk.43.hc_head_fn/base/scale/norm.weight`. Before the fix, GPU head preparation disagreed with the reference (`mix_max_abs` about `9.5`, `weight_max_abs` about `0.21`). Root cause: `hc_head_fn` rank-localization treated each rank as owning one contiguous `4 * shard_cols` span, but the HC vector is row-major `[4][4096]`; each rank owns `512` columns inside every HC row. `open_shared_output_head` and `load_mtp_output_head` now slice with `global_c = hc_row * 4096 + rank * 512 + local_col`. After the fix, diagnostic mismatch collapsed to float noise (`mix_max_abs <= 4.8e-7`, `norm_max_abs <= 1.4e-7`). Clean pod build passed and the clean acceptance harness still reports `71` pairs with `0` same-index and `0` next-index matches. | Output-head prep is now reference-clean, but the B1 blocker remains below the head. Next MTP sprint should apply same-activation localization to layer-43 body math/state, starting at attention projection/raw-SWA and then post-attention FFN output, before building the K-wide verifier. |
| 2026-05-30 | Sprint 591 rejected layer-43-only inverse attention-head RoPE as the MTP acceptance fix. | A temporary candidate applied inverse RoPE to `r.d_attn_heads` after MTP layer-43 raw-SWA attention and before attention output projection, matching an upstream semantic boundary in `research/ds4/ds4.c`. The pod build passed and the candidate changed draft tokens (`draft[:12]` changed from `[112865, 5743, 8373, 13151, ...]` to `[18560, 5743, 80610, 84941, ...]`), proving the probe was active. Acceptance remained `71` pairs with `0` same-index and `0` next-index matches. The candidate was removed and the clean pod build passed. | Do not chase layer-43-only inverse-head RoPE further as the immediate B1 fix. Continue below-head localization by validating MTP layer-43 dense/projection correctness and HC body controls on synthetic or actual activations. |
| 2026-05-30 | Sprint 592 cleared MTP layer-43 F8 dense pack/orientation as the zero-acceptance cause. | A temporary `dense_compute_all_f8` run initially exposed a real diagnostic-path bug: `run_layer(43)` used the MTP contract but inherited the main `pack_dir`, so descriptor-backed diagnostics failed on `blk.43.attn_kv_latent.weight`. The durable fix sets `mtp_opt.pack_dir = opt.mtp_pack_dir ?: opt.pack_dir` in both the serving draft path and appliance scaffold. With that fix, all MTP layer-43 F8 dense tensors passed the CPU dense oracle (`oracle_max_abs <= 3e-8`, `oracle_bad = 0`, `repeat_bad = 0`). The deterministic acceptance harness still reported `71` pairs with `0` same-index and `0` next-index matches. The temporary oracle hook was removed and the clean pod build passed. | Do not re-test MTP dense F8 pack/orientation unless inputs change. Next B1 work should localize actual activation/state semantics below the dense matmuls: HC controls, raw-SWA/ring state, post-attention handoff, and FFN activation sequence. The K-wide verifier and throughput A/B remain blocked until acceptance is nonzero. |
| 2026-05-30 | Sprint 593 cleared MTP HC-current control/reduction as the zero-acceptance cause. | Three MTP-only probes built and ran on the pod. Centralized HC-current (`tp_hc_current_allreduce_gate=false`) stayed `0/71` and kept the baseline first draft tokens. Reference HC reduction (`reference_hc_reduce_gate=true`) changed draft tokens (`[112865, 15387, 8373, 13151, ...]`) but still stayed `0/71`. HC-current full parity passed for layer 43 with `diff_bad=0` and `max_abs_diff` around `1.4e-6` to `3.3e-6`; acceptance remained `0/71`. Temporary probe code was removed and the clean pod build passed. | Do not re-run HC all-reduce/full-parity/reducer-choice probes unless HC control loading changes. Next B1 sprint should capture actual activations across raw-SWA, attention output, post-attention handoff, and FFN output; the K-wide verifier and throughput A/B remain blocked until acceptance is nonzero. |
| 2026-05-30 | Sprint 594 ruled out stale/frozen MTP layer-43 activations and the cheap raw-row frontier mismatch. | Temporary MTP-only activation logging around `run_layer(43)` built on the pod. Acceptance stayed rejected (`71` pairs, `0` same-index and `0` next-index matches), but real rank-0/slot-0 activation samples were finite and input-responsive. Raw-SWA metadata was coherent at the sampled frontiers: positions `0/1/2/3/7/15/31` mapped to raw rows `0/1/2/3/7/15/31` with valid rows `1/2/3/4/8/16/32`. Samples included nonzero prologue, raw-SWA, attention-head, post-attention, next-hidden, and final-HC values; no stale/zero/frozen state was observed. Temporary logging was removed and the clean pod build passed. | Do not run the raw-row physical-position probe without new evidence. Next B1 work should compare layer-43 body semantics below the dense/HC/head paths: raw-SWA attention math/window semantics, attention-output handoff, and routed-FFN activation sequence. Throughput A/B remains meaningless until acceptance is nonzero; debug runs are intentionally low tok/s because they add rejected draft work and host-sync diagnostics. |
| 2026-05-30 | Sprint 595 cleared MTP layer-43 raw-SWA attention math/windowing. | A temporary same-point CPU oracle copied rank-0/slot-0 Q heads, raw-SWA rows, rank-0 sink logits, and GPU attention heads after `run_layer(43)`. The first attempt exposed a diagnostic pitfall: `ranks[0].d_attn_sinks` can be overwritten by later rank sink broadcasts, so the corrected oracle copied sinks from `shared_hc_controls->d_attn_sinks[43]`. With that correction, raw-SWA attention matched the CPU sink-aware softmax reference at sampled frontiers `1/2/3/4/8/16/32` valid rows (`max_abs` about `9.5e-7` to `3.6e-6`). Acceptance remained `0/71`. Temporary oracle code was removed and the clean pod build passed. | Do not re-test raw-SWA attention unless its inputs change. The zero-acceptance blocker is now downstream of raw-SWA attention and upstream of the already-cleared head: attention-output projection, post-attention/FFN handoff, routed-FFN activation sequence, or MTP-specific semantics around those boundaries. |
| 2026-05-30 | Sprint 596 punted active MTP work and documented the non-working state. | A temporary post-attention residual oracle was started and built, but the run was intentionally interrupted by user decision before producing usable evidence. The temporary source change was removed. `MTP_IMPLEMENTATION.md` now records that V100 TP/EP MTP does not work: the integrated draft path runs and preserves the main token stream, but deterministic draft acceptance remains `0/71`. `README.md` now documents why the V100 LP and EP MoE configurations are structurally hard to saturate and why MTP is not currently a supported speed path. | Do not continue MTP localization or build the K-wide verifier unless explicitly reopened. Treat V100 LP/EP appliance work as research scaffolding; return to non-MTP steering items only if the V100 track is resumed. |
| 2026-06-11 | Sprint 597 planning REOPENED the V100 track on the B2 (EP-overhead) path and corrected three load-bearing code-model claims. | A codebase review argued the README's "MoE compute density is the structural blocker" epitaph does not match the record: actual expert math is ~`0.2-0.3` ms of the `9.4` ms/layer EP stage (Sprint 200 vs 581), and the weight-streaming roofline (~`10` ms/step) puts the ceiling near ~`1,000+` tok/s vs the measured `26.8`. Three-model consensus planning (opus-4.8 / gpt-5.5 / gemini-3.1, drafts + cross-critiques, all source-verified) corrected the intent: (1) the promoted full-capture path has NO per-layer host route-count readback — it runs a fixed-capacity `192`-row executor envelope (~3x padding, Sprint 542); (2) the promoted EP return is 56 per-pair `copy_f32_kernel` UVA remote-load launches/layer (`decode_loop.cu:1174-1195`), NOT NCCL — and nothing accounts for SYS crossings on the 12 non-NVLink cube-mesh pairs; (3) Sprint 396 showed NCCL 2.96x FASTER than custom peer-doubling, so custom transport is not pre-proven. | Sprint 597 (measurement-only): reproduce the 581 anchor, per-pair transport/SYS audit, EP sub-stage decomposition with nsys-on-promoted-graph as the decision authority, then pick the 598+ B2 stage order from the measured dominator (branch table in SPRINT-597.md). B2 stage menu + go/no-go gates in SPRINT-597-DEFERRED.md. MTP stays punted; PP stays dead. |
| 2026-06-11 | Sprint 597 EXECUTED (full DoD): the EP overhead is the SYS-crossing return transport; B2-C leads 598. | Environment rebuilt from scratch (workspace had been wiped): s597 pack/contract regenerated, probe smoke passed. Re-anchored: full-capture `73.59` tok/s decode-domain steady (`59.3` wall — the old `26.8` was a harness artifact: 128 simultaneous connects vs listen backlog 16; per-request decode reproduced s581 within 2%); eager `18.165` ms/layer-step, EP `11.136` ms (61.3%), HC-current drifted `1.10 -> 5.55` ms post-MTP churn. Decomposition (new flag `DS4_V100_TP_EP_EP_STAGE_PROFILE`, flag-off byte-identical, flag-on `-1.85%`, residual `0.4%`; three instruments concur): EP-return per-pair copies = `6.92` ms = **81% of the 8.52 ms EP window**, with 24/56 directed pairs crossing SYS at ~`2` ms each in situ (microbench isolated SYS only 2-3.5x — congestion is the multiplier); expert math `~0.25` ms (~3%); padded 192-row executor tax `~0` (launch-bound); barriers `0.85` ms; eager NCCL-broadcast control `0.68` ms/layer (10x cheaper than the promoted graph copies). Hypothesis adjudicated ~3%/97%. | Sprint 598 = B2-C: eliminate SYS from the EP return — candidate 1 captures NCCL broadcast in-graph (0.68 ms control), candidate 2 static one-hop NVLink relay (~0.2 ms bound); must beat both controls; projected ~2.3-2.6x decode-domain. Then B2-D per-pair events (0.85 ms bound). B2-A dropped (tax ~0). Follow-ups: HC-current 5.55 ms regression localization; harness upstreaming; see SPRINT-597-FOLLOWUPS.md. Report: SPRINT-597-REPORT.md. |
| 2026-06-11 | Sprint 598 PROMOTED B2-C: NCCL-in-graph EP return; **2.28x decode-domain (162.06 tok/s)**. | C1 capture probe proved grouped per-source `ncclBroadcast` captures+replays in the 8-rank graph (NCCL 2.19.3, 0.72 ms/round). Promoted behind `DS4_V100_TP_EP_EP_RETURN_TRANSPORT` (launcher default `nccl`, `copy` rollback): EP-return `6.92 -> 0.611` ms/layer, layer replay `10.2 -> 4.25` ms, decode-domain `71.21 -> 162.06` tok/s (2.28x same-harness; 2.20x vs s597 anchor), wall `108.50` (1.83x), tolerance bit-exact vs the s597 control, nsys-proven zero SYS-class kernels. Warm-ups closed s597 deviations (identity re-proof; backlog 16->256; harness fixes upstreamed). C2 (one-hop relay) not needed. The s597 cycle target as written (>=80 tok/s) is exceeded. | Sprint 599: pre-EP prefix is the new dominant cost (~1.78 ms/layer of the 4.25 ms replay: HC-current + attention; HC-current measured 5.55 ms in eager) + shared swiglu_down 0.78 overlap + route_plan_pack 0.53 + B2-D barriers 0.30. Honest cycle stretch: 3x of re-anchored baseline ≈ 220 decode-domain. |
| 2026-06-11 | Sprint 599 adjudicated the stretch: 220 NOT reached, measured-reachable, gated on a LATENT ORDERING HAZARD in the promoted path. | Post-C1 decomposition: layer is wait-bound (GPU busy 2.94 of 4.25 ms); top stages prefix_attn_output 0.723 / swiglu_down 0.793 (1,792 per-slot UVA remote loads/layer, partly SYS) / ep_return 0.614 / route_plan_pack 0.517. Five swiglu-exchange replacements gained +11.5-17.9% (186.33 / 197.17 decode-domain) but ALL failed tolerance — including bit-identical-mechanics batched loads at 1/32 launches — proving a timing-dependent missing dependency downstream that the promoted path's launch-storm slowness masks. C-B early-return/per-rank-ordering: tolerance 1.0/1.0 but +0.5% (noise) — opt-in, not promoted. No promotions; clean baseline re-measured 167.19 decode-domain / 112.70 wall. Floor math: swiglu fix -0.50 (demonstrated) + barriers -0.26 (clean) + route-plan shadow -0.3..0.5 (unattempted) + prefix pool 1.54 sums past the 3.13 ms budget for 220. | Sprint 600 leads with the hazard root-cause (also a promoted-path correctness debt): find the missing dependency edge consuming the swiglu exchange, fix it, prove under timing perturbation, then re-apply the fast exchange + remaining levers and re-adjudicate 220. |
| 2026-06-12 | Sprint 600 root-caused the hazard: a race INSIDE captured NCCL collectives; fires on the promoted path (~1/256 steps). Missing-engine-edge hypothesis FALSIFIED. | Eight-way elimination matrix (engine edges, payload staleness ×3 verifiers, cross-replay, atomics, NCCL 2.27.7, protocol, eager/captured mixing — all eliminated). Trigger law: firing scales with intra-replay pacing; ~600 µs/layer delay anywhere restores bit-identity (the promoted path's slowness IS the protection). Whole-batch corruption events up to 12/32 token flips, localized per-step by the new first-divergence checksums. Fix candidates: dedicated head comm blocked by the pod 64 MiB /dev/shm k8s default (33.5 MiB/comm — infrastructure, orchestrator lifts it in 601); host-side head reductions + NCCL 2.27.7 + Ring: race unaffected. No promotions; baseline 170.18 decode-domain / 117.08 wall. Program target clarified by user: ≥50 tok/s PER SLOT (step ≤20 ms; per-slot is slot-flat; needs S≤16 + MTP ~2x on the credible path). | Sprint 601: (1) lift /dev/shm via emptyDir medium:Memory, retest dedicated-comm isolation of the race; (2) NCCL-free EP window (batched exchange + s597 relay-table return; ~195-205 projection); (3) empirical scaling curve S=1/4/8/16/32; (4) then the MTP-reopen decision gate. NVIDIA escalation package assembled. |
| 2026-06-12 | Sprint 601: in-NCCL fix space exhausted; race localized to hc-class collectives; promoted path fired TOKEN-FLIPPING events (2/3 controls); relay transport built (+23.8% demonstrated, unpromotable until the race dies); scaling curve measured. | Phase A comm isolation FAIL at all granularities (shm 16Gi, comms init fine, identical divergence signature). Phase B relay EP return: peer-write NVLink + balanced one-hop via dst^4 for SYS pairs, 0.24-0.26 ms/layer (vs 0.61 NCCL), tolerance 1.0/1.0; relay+batched = 208.18 decode-domain; race verdict FAIL — survives in hc allreduces/allgather, router allreduce, full-current broadcast, post-attn allgather (same step-2 locus as the promoted path's events). Phase C no promotion: relay raises event rate 1.5 vs 1.0/run (pacing exposure). Phase D curve: per-slot 8.11/7.67/7.96/7.00/5.28 at S=1/4/8/16/32; step floor ~123-130 ms flat to S=8, launch/wait-bound (only swiglu copies + GEMMs scale with S). ≥50/slot math: MTP-required multiplier 6.3-7.1x at current floor — base must reach ~40-60 ms first. | Sprint 602: fully NCCL-free decode graph — ring-order-exact kernel reductions for hc/router collectives. Simultaneously the correctness fix, the relay+batched unlock (+23.8% demonstrated), and the prerequisite for prefix compaction / route-plan shadow / cross-layer consolidation. MTP gate re-sequenced after 602 + one major step lever. |
| 2026-06-13 | Sprint 602: zero-NCCL captured graph BUILT, BIT-EXACT, TOKEN-RACE-FREE; defaults flipped to it as the correctness baseline (orchestrator); perf reclaim deferred to 603. | 9-class kernel-collective set; dot proof 0 nccl nodes; fold probe cracked NCCL's exact accumulation (real auto ring `0 3 2 1 5 6 7 4` — the documented NO_SYS_RING was never exported — + minChunk rule), so the s597 bit anchor SURVIVES (tolerance 1.0/1.0, 8192/8192 everywhere). LL census: 6/6 token-bit-exact, 0 token events, 0.17 checksum-only/run vs 1.0/run with token flips on captured-NCCL configs; counter-proof: kernel-only with EP-return NCCL still captured fired a token event. Batched exchange exonerated (s599 failures were the NCCL race). Sprint gates: all PASS except perf (~153 vs ≥194.4 — 16 rank-joins/layer ≈ 1.5 ms/layer mask relay+batched). Launcher defaults now kernel/relay/batched (rollback envs retained; ~-9% accepted for a trustworthy control). Residual: rare checksum-only late-step flicker in the s602 site sync, Simple-stress only. ≥50/slot: required MTP multiplier 9.3-9.4 at this floor. | Sprint 603: join reclaim — replace the 16 all-rank rank-stream joins with per-collective producer-consumer edges that keep token-race-zero (census gate); recover toward ≥194-208; hunt the checksum flicker; then prefix compaction + route-plan shadow; MTP gate after. |
| 2026-06-13 | Sprint 603: edges built (+6.4%, unpromoted on the +15% gate); s602 "token-race-zero" FALSIFIED at larger n — the residual is a rank↔dense gap, not the s602 site sync. | Per-collective edges Simple-stress 3/3 bit-identical but only ~25% of the join pool reclaims (transitive rendezvous bounds it). At larger n every rank-stream-only sync (edges 1/6, join 1/3) fires rare token events; only the full rank+dense barrier is clean (6/6); the derivable bcast dense-WAR (DENSE_GUARD) was falsified. Event rates swing 3-5x day-to-day → small-n gates invalid. | Sprint 604: root-cause the rank↔dense hazard. |
| 2026-06-13 | Sprint 604 ROOT-CAUSED AND FIXED the rank↔dense hazard (the program's gating correctness bug); DENSE_FIX promoted default-on. | Carrier = cross-rank dense→rank RAW on the attention-output allgather (attn_output_a.d_out: written on src dense stream attention_output.cu:48, read cross-rank on dst rank stream :87-98; the :51 edge orders same-rank only). NOT the s603-falsified bcast WAR. Method: a deterministic amplifier (DENSE_HAZARD_AMP, dense-stream busy-wait) drove it 0→~100% single-run token corruption (soaks → 1-run gates) + a codex static audit naming the same carrier; both converged. Fix = minimal cross-GPU dense↔rank edge (fb's dense ordering minus the redundant 8×8 rank join), near-zero cost. Proof: 34-run alternating soak fix-on 17/17 clean / fix-off 7/17 token-corrupt (prior default = 41% corrupt); composes with edges (2/2). Clean floors ~177 ms (edges+fix S=8) / ~189 (join+fix). | Sprint 605: promote edges+fix (correctness objection gone) after a clean soak; then the step-floor levers (prefix compaction ~1.1, route-plan shadow ~0.45, cross-layer consolidation) to drive 177 ms → the ~40-60 ms MTP-reachable floor; THEN the MTP gate. ≥50/slot arithmetic unchanged (MTP ~8.8-9.4) but on a valid foundation; the bug-chase is over. |
| 2026-06-14 | Sprint 605 PROMOTED edges+fix; opened the step-floor campaign; found the heaviest prefix stage is DMA-transport-bound (overlap, not launch compaction, is the lever). | edges+fix: 5/5 amplifier gate (attn_out_a ×3 + pre_compose late-step) + 32/32 soak clean → launcher default join→edges (rollback retained). Clean-base floors: S=8 199 ms (5.0 tok/s/slot), S=32 222 ms (4.5/slot); routed GEMMs <5% of the step (launch/sync/transport-bound, s601 reconfirmed). Microbatch VRAM gate resolved in its favor (~180× headroom; the 28.8 GiB is shared per-layer KV state, not duplicated). Phase C lever attn_output gather8 (72→16 launches/layer): correctness PASS (incl. amp@20us at the carrier site) but perf REGRESSED ~10% — the 64 memcpy2D are NVLink copy-engine DMA, gather kernels serialize on SMs; launch count is NOT the bottleneck there. NO promotion of the lever. ≥50/slot: required MTP mult ~10 @ S=8; 0 captured this sprint but base is now correct+stable+characterized. | Sprint 606: microbatch ping-pong (split S into 2 half-batches, overlap one's transport-bound prefix + EP window with the other's GEMM compute) — the highest-ceiling lever, VRAM-feasible, now clearly RIGHT for a transport-bound step; needs graph-split / in-graph event choreography + split router/route-count, amplifier-gated. Then route-plan shadow + rendezvous merge, then the MTP gate (s604 is a fresh 0/71 lead). |
