...
tools/ds4-v100-tp-ep-full-layer-smoke:
	@echo "tools/ds4-v100-tp-ep-full-layer-smoke requires a CUDA build"
	@exit 2
...
tools/ds4-v100-tp-ep-full-layer-smoke: tools/ds4-v100-tp-ep-full-layer-smoke.cu ds4_v100_tp_runtime.cu ds4_v100_tp_runtime.h ds4.h $(CPU_CORE_OBJS) kernels/turbomind/ggml-turbomind/include/ggml-turbomind-api.h kernels/v100/common.cuh kernels/v100/dense.cuh kernels/v100/hc_mix.cuh kernels/v100/hc_shards.cuh kernels/v100/norm.cuh kernels/v100/compose.cuh kernels/v100/router.cuh kernels/v100/diagnostics.cuh kernels/v100/fill_pack.cuh kernels/v100/attention.cuh engine/decode_loop.cu engine/hc_current.cu engine/hc_final.cu engine/attention_projection.cu engine/compressed_kv_step.cu engine/attention_read.cu engine/attention_output.cu engine/post_attention_ffn.cu engine/router_step.cu engine/router_plan.cu engine/ep_dense.cu engine/ep_executor.cu engine/ep_compose.cu engine/layer_decode.cu engine/layer_runner.cu engine/token_major_loop.cu
	$(NVCC) $(NVCCFLAGS) --std=c++17 -I. -o $@ tools/ds4-v100-tp-ep-full-layer-smoke.cu ds4_v100_tp_runtime.cu $(CPU_CORE_OBJS) $(CUDA_LDLIBS) -lnccl

tools/ds4-v100-tp-ep-dense-cache-smoke: tools/ds4-v100-tp-ep-dense-cache-smoke.cu
	$(NVCC) $(NVCCFLAGS) --std=c++17 -I. -o $@ $< $(CUDA_LDLIBS)

tools/ds4-v100-tp-ep-int8-compressor-workbench: tools/ds4-v100-tp-ep-int8-compressor-workbench.cu kernels/tc-grid/kernels/v12_kernels.cuh kernels/tc-grid/kernels/v13_kernels.cuh kernels/tc-grid/kernels/mma_sm70.cuh
