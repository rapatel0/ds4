...
| In progress | MTP (B1) — the EP-fill throughput bet | EP 65% | EP is 65% of decode *because* of sub-1-token-per-expert. MTP (verify K draft tokens) makes each step see (K+1)x tokens -> fills grouped-GEMM tiles, raises occupancy, and amortizes the all-to-all and per-step host orchestration -> attacks the 65% and the utilization waves at the root. Sequence: (a) **weight integration** -- Sprint 582 confirmed the 32-family MTP tensor set in the safetensors and found the expert weights are **I8+E8M0** (not mxfp4 as the plan assumed), so the converter must **re-quantize I8+E8M0 -> MXFP4** (draft model -> lossy OK, ~Q4_K parity); converter + `tp-ep-pack-contract.c` layer-43 (Sprint 583), `runtime_pack.cu` binding + sidecar delete (Sprint 584); (b) MTPBlock.forward in `engine/` (1 sprint, Phase A); (c) TP/EP specdec accept/reject loop (the actual throughput sprint, multi-sprint, opts into perf). See `MTP_IMPLEMENTATION.md`. | High |
