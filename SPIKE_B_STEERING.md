...
| Punted | MTP (B1) — the EP-fill throughput bet | EP 65% | EP is 65% of decode *because* of sub-1-token-per-expert. MTP (verify K draft tokens) makes each step see (K+1)x tokens -> fills grouped-GEMM tiles, raises occupancy, amortizes the all-to-all -> attacks the 65% at the root. **(SPRINT 584 RESULT): weight integration + MTP forward body DONE + validated; specdec loop is Sprint 585.** Sprint 584 built + validated, on the pod, the entire EP=8 MTP weight integration (steps 1-4) AND the MTP forward body executing through the multi-rank EP decode (`decode_pass=1`, EP-split experts, serving byte-intact). All weight loaders built (`open_mtp_expert_bindings`, `open_mtp_nonexpert_bindings`, `load_mtp_hc_layer43`, `load_mtp_dense_layer43`, `load_mtp_output_head`; `kLayers`->44 + guards + struct `[44]`). The ONLY remaining piece is the **TP/EP speculative-decode loop** -- a from-scratch multi-rank serve-path decode driver (draft K via the MTP forward -> verify K+1 via the existing `ds4_replay_verify_token_block` -> accept/reject), integration point `appliance/http_server.cu` generation loop, validated by a live serving run (acceptance rate + throughput). Scoped as **Sprint 585** (`docs/sprints/SPRINT-585.md`); to run as a fresh focused session. **(SPRINT 585 PROGRESS):** the full EP MTP draft path is now built + committed (1.1 head, 1.2a proj load, 1.2b prologue compute, state-split) and validated as RUNNING on the pod. Key correction: the verify is NOT `ds4_replay_verify_token_block` (single-slot replay binary, unreachable from the 32-slot appliance); the appliance has no replay runtime, so the EP-fill verify must be a new (K+1)-wide block-verify in `run_token_major_serving_loop` (Phase 2). Also: the LP sidecar (`engine/mtp_step.cu`) was never working, so it is NOT a valid draft oracle. **State-split PROVEN**: with MTP enabled the appliance serves byte-identical tokens to mtp-off (main hidden snapshot/restored around the MTP draft). **Gating issue: MTP draft acceptance = 0** — the draft (finite, input-responsive, logits ~20-32) does not match the main model's next token, so the EP MTP draft is numerically incorrect. **(SPRINT 586 RESULT):** the per-cache-slot MTP raw-window state repair is implemented and build-valid; layer 43 now sees `mtp_raw_valid_rows` grow from `1` upward across prefill/continuation instead of being pinned to HTTP loop step `0`. Acceptance remains rejected (`71` pairs, same-index `0`, next-index `0`), so this rules out "valid_rows stuck at 1" as the sole cause. **(SPRINT 590 RESULT):** same-activation CPU reference found and fixed a real output-head HC-control rank-slicing bug in `open_shared_output_head` / `load_mtp_output_head` (`hc_head_fn` must slice row-major `[4][4096]` as `hc_row * 4096 + rank * 512 + local_col`, not one contiguous `4 * shard_cols` span per rank). The MTP head preparation is now reference-clean (`mix_max_abs <= 4.8e-7`, `norm_max_abs <= 1.4e-7`), but acceptance remains rejected (`71` pairs, same-index `0`, next-index `0`). Next: same-logical-point numerical localization below the head, starting with layer-43 attention projection/raw-SWA and post-attention FFN output. **(SPRINT 591 RESULT):** a layer-43-only inverse attention-head RoPE candidate was tested because upstream applies inverse RoPE between raw-SWA attention and grouped output projection. The probe was active and changed draft tokens, but acceptance stayed rejected (`71` pairs, same-index `0`, next-index `0`), so the candidate was removed. **(SPRINT 592 RESULT):** layer-43 F8 dense pack/orientation was cleared. A temporary dense oracle first exposed a real MTP diagnostic-path bug (`run_layer(43)` used the MTP contract while descriptor-backed diagnostics inherited the main `pack_dir`); the durable fix sets `mtp_opt.pack_dir = opt.mtp_pack_dir ?: opt.pack_dir` in the serving draft path and appliance scaffold. With that fixed, all MTP layer-43 F8 dense tensors passed the CPU oracle (`oracle_max_abs <= 3e-8`, `oracle_bad=0`, `repeat_bad=0`), while acceptance remained `71` pairs with `0` same-index and `0` next-index matches. **(SPRINT 593 RESULT):** MTP HC-current/control probes were cleared. Centralized HC-current stayed `0/71` with baseline draft tokens; reference HC reduction changed draft tokens but stayed `0/71`; layer-43 HC all-reduce-vs-full parity passed with `diff_bad=0` and `max_abs_diff` about `1.4e-6` to `3.3e-6`. Do not re-test dense F8 pack/orientation or HC all-reduce/reducer-choice unless their inputs change. Continue with raw-SWA/ring state, attention output, post-attention handoff, and FFN activation semantics. The (K+1)-wide block-verify throughput loop + the 26.8-baseline delta are only worth building once acceptance > 0. **PUNT (2026-05-30):** per user direction, stop active MTP work. The V100 TP/EP MTP draft path does not work (`0/71` deterministic acceptance) and is documented in `MTP_IMPLEMENTATION.md`; do not resume without an explicit reopen. Below text is historical.

**Current MTP localization note (Sprint 594):** temporary activation capture
around layer 43 showed finite, input-responsive prologue, raw-SWA,
attention-head, post-attention, next-hidden, and final-HC samples. The sampled
raw frontier was coherent (`position == raw_row` at `0/1/2/3/7/15/31`, valid
rows `1/2/3/4/8/16/32`), while deterministic acceptance stayed `0/71`. Do not
run the raw-row physical-position probe unless new evidence points there. The
next B1 target is a semantic comparison of raw-SWA attention math/windowing,
attention-output handoff, and routed-FFN activation order. Throughput metrology
remains blocked until acceptance is nonzero; low tok/s in current MTP runs is
expected because the harness adds rejected draft work plus diagnostics.

**Current MTP localization note (Sprint 595):** layer-43 raw-SWA attention
math/windowing is cleared. A corrected same-point CPU oracle matched the GPU
attention heads for rank 0 / slot 0 across valid-row frontiers
`1/2/3/4/8/16/32` with max error about `9.5e-7` to `3.6e-6`; acceptance
remained `0/71`. Do not re-test raw-SWA attention unless its inputs change.
Continue downstream: attention-output projection, post-attention/FFN handoff,
and routed-FFN activation sequence.
