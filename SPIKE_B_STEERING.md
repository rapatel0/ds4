...
| In progress | MTP (B1) — the EP-fill throughput bet | EP 65% | EP is 65% of decode *because* of sub-1-token-per-expert. MTP (verify K draft tokens) makes each step see (K+1)x tokens -> fills grouped-GEMM tiles, raises occupancy, amortizes the all-to-all -> attacks the 65% at the root. **(a) Weight-pack: DONE + validated (Sprint 583).** The experts are packed-FP4+E8M0 (not lossy-I8 as first assumed) -> a **lossless** re-pack to mxfp4/f8_e4m3_b128. The only new artifact is the converter `tools/mtp-pack-fragment.c` (reads the safetensors, re-packs all families, stacks experts, emits a convention-compliant GGUF + Sprint-002 manifest; re-pack round-trips 0-mismatch on real weights). The full pipeline (pack.c -> turbomind-pack --layer 43 -> tp-ep-pack-contract) handles layer 43 **generically with no code changes** -- `expert_rows=24` (EP8), dense/control sharded; the steered "contract layer-43 extension" is a no-op. Remaining weight bits: `appliance-pack --layer 43` real shard emission + `runtime_pack.cu` layer-43 bind + sidecar delete (mechanical). **(b)** MTPBlock.forward in `engine/` (Phase A, kernels). **(c)** TP/EP specdec accept/reject loop (Phase B, the throughput sprint, multi-sprint, opts into perf). See `MTP_IMPLEMENTATION.md`. | High |
