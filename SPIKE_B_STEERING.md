...
| 1 | C1 captured-compose ordering dependency (trace + fix) | full capture | Sprint 578 ruled out compose accumulation (default uses deterministic `ep_pack`) and route ordering (deterministic by construction) -- the live hypothesis is a missing intra-graph event dependency in the captured compose (eager serializes with `cudaStreamSynchronize`, illegal in capture). Trace with `nsys` (lighter than compute-sanitizer, which OOMed; may run on the full appliance) to find the captured compose-combine running concurrently with the cross-GPU copy/broadcast without a dependency edge, or extend `tp_ep_decode_top1_logit` per-stage inside the captured region. Then add the missing `cudaStreamWaitEvent`/event dep (or force a deterministic combine), rebuild, and confirm the full-vs-full logit floor collapses toward the eager floor. | Med-High |
